...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
      ...
      SUBROUTINE GAMME_CE( sinbeta, Tc, T24,  &
                           PPFD0, PPFD24, Wind,  &
                   Humidity,Cantype,LAI,Pres,DI,  &
                   Ea1Canopy,EatiCanopy )
...
! input
!      INTEGER,INTENT(IN) :: jdate, jhour, NrCha, NrTyp
      REAL,INTENT(IN) :: Cantype
...
! output
      REAL,INTENT(OUT) ::  Ea1Canopy(n_mgn_spc), EatiCanopy(n_mgn_spc)

! local variables
      INTEGER :: i, JJ, KK, K, Day, S
...
!,Beta
      ...
      real :: Ea1p99sunLayer(Layers), Ea1p99shdLayer(Layers)

      REAL :: SHour, Solar, Maxsolar,  &
...
!        Hour  = JTIME + LONG /15
     ...
     ! Solar hour
!!     Hour = jhour + long / 15.0
!!
!!     IF ( SHour  .LT. 0.0 ) THEN
!!        Hour = Hour + 24.0
!!        Day  = Day  -  1
...
!      SH         = 0.
      ...
      do i = 1, layers
         ...
         Ea1p99sunLayer(i) = Ea1p99(SunPPFD(i), PPFD24*0.5, pstd_Sun) &
                           * Sunfrac(i)

         Ea1p99shdLayer(i) = Ea1p99(ShadePPFD(i), PPFD24*0.16, pstd_Shade) &
                           * (1. - Sunfrac(i))
      enddo

      topt = 312.5 + 0.6 * (T24 - 297.0)
      eopt = EXP(0.1 * (T24 - 297.))

      do s = 1, n_mgn_spc

         DO i = 1, layers

!        Ea1tLayer(i) = Ea1t99(Sunleaftk(i), T24, S) *  &
!                        Sunfrac(i) +  &
!                       Ea1t99(Shadeleaftk(i),T24, S) *  &
!                         (1.-Sunfrac(i))
...
!                              PPFD24*0.5,pstd_Sun)* Sunfrac(i) +  &
!                       Ea1p99(ShadePPFD(i),PPFD24*0.16,  &
!                           PPFD24*0.16,pstd_Shade)*(1-Sunfrac(i))  

!            Ea1Layer(i)  = Ea1t99(Sunleaftk(i), T24, S) *  &
...
!                              PPFD24*0.16,pstd_Shade)*(1.-Sunfrac(i))
            ...
            Ea1Layer(i)  = Ea1t99(Sunleaftk  (i), topt, eopt, S) * &
                           Ea1p99sunLayer(i)                       &
                         + Ea1t99(Shadeleaftk(i), topt, eopt, S) * &
                           Ea1p99shdLayer(i)

            EatiLayer(i) = Ealti99(S, Sunleaftk  (i)) *     Sunfrac(i)  &
                         + Ealti99(S, Shadeleaftk(i)) * (1.-Sunfrac(i))
...
!     Ea1tCanopy = SUM( Ea1tLayer(:) * VPslwWT(:) * VPgausWt(:) )
      ...
      Ea1Canopy (s) = SUM( Ea1Layer (:) * VPslwWT(:) * VPgausWt(:) ) * Cce * LAI
      EatiCanopy(s) = SUM( EatiLayer(:) * VPslwWT(:) * VPgausWt(:) )

   enddo


!      write(*,*) SunPPFD(1), PPFD24, PPFD240, Ea1p99(SunPPFD(1),PPFD24*0.5,  &
!                              PPFD240*0.5,pstd_Sun), sunfrac(1)
...
!--------------------------------------------------------------------
      ...
      ELSE
         DO i = 1, Layers
            Weightgauss(i) = 1. / real(Layers)
            Distgauss(i)   = (real(i) - 0.5) / real(Layers)
            Distgauss2(i)  = real(i) / real(Layers)
        ENDDO
      ENDIF
...
! Pa m-1  (humidity profile for T <278)
      ...
      WHERE (Wsh(:) < 0.001) 
         Ws(:) = 0.05
      ELSEWHERE
         Ws(:) = max( Ws0*LOG(Wsh(:))/LOG(Cheight-Canopychar(16,Cantype)*Cheight), 0.05 )
      ENDWHERE
...
!!      ENDIF
      ...
      ! Nobel page 366
      GHforced = 0.0259 / (0.004 * sqrt(Llength / Ws))
...
! > -.5 incipient,  mild or no drought; < -4 extreme drought
      REAL,PARAMETER :: DIhigh = -0.5, DIlow = -5.0
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
      ...
      CalcEccentricity = 1. + 0.033 * COS(2.*3.14159*(Day-10.)/365.)
...
!ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
      ...
      REAL :: Tleaf, Ambvap, LatHv, GH, StomRes,  &
              TranspireType, LeafRes, Vapdeficit, LeafLE
...
!--------------------------------------------------------------------
      ...
      Vapdeficit = (rhovsl(Tleaf-273.15) - Ambvap)
...
!                 leaf resistence (s m-1)
      ...
      LeafLE = max( TranspireType * LatHv * Vapdeficit / LeafRes, 0.0 )
...
!ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
      ...
      FUNCTION Ea1p99(PPFD1, PPFD24, PSTD)

      IMPLICIT NONE

      real, intent(in) :: PPFD1, PPFD24, PSTD
      REAL             :: Alpha, C1, Ea1p99
!--------------------------------------------------------------------
