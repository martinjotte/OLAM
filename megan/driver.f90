...
contains
  ...
  subroutine alloc_megan(mwl, mwa, mlandflux)
...
!   allocate(mgn_2_nr_map(n_nr_emis))
    ...
    allocate(megan_emis(mlandflux,n_mech_spc))
...
!   enddo
  ...
  subroutine megan_avg_temp()
    ...
    !$omp do
    do iwl = 2, mwl
       avg_temp(iwl) = avg_temp(iwl) * so + land%can_temp(iwl) * si
    enddo
    !$omp end do
  ...
  subroutine megan_driver(mrl)
    ...
    use misc_coms,   only: io6, iparallel, current_time
