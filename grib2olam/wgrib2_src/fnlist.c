...
//   {"gctpc",f_gctpc, inv, 1, " X=0,1 use gctpc library (testing)", 100},
   {"gdt",f_gdt, inv, 0, "contents of Grid Definition Template", 200},
   {"get_byte",f_get_byte, inv, 3, "get bytes in Section X, location Y (1..N), number of bytes Z (decimal format)", 100},
   {"get_hex",f_get_hex, inv, 3, "get bytes in Section X, location Y (1..N), number of bytes Z (bytes in hexadecimal format)", 100},
   {"get_int",f_get_int, inv, 3, "get 4-byte ints in Section X, location Y (byte), number of ints Z", 100},
...
//   {"proj4_ll2ij",f_proj4_ll2ij, inv, 2, "x=lon y=lat, converts lon-lat (i,j) using proj.4 (experimental)", 100},
   {"radius",f_radius, inv, 0, "radius of Earth", -1},
   ...
   {"set_ens_num",f_set_ens_num, misc, 3, "convert PDT 0,1 -> 1, 8,11 -> 11, X=code table 4.6 Y=pert num Z=num ens members", 100},
   ...
   {"set_hex",f_set_hex, misc, 3, "set bytes in Section X, location Y (1..N), bytes Z (a|a:b:c) in hexadecimal", 100},
   {"set_ijval",f_set_ijval, misc, 3, "sets grid point value X=ix Y=iy Z=val", 100},
   {"set_int",f_set_int, misc, 3, "set 4-byte ints in Section X, location Y (1..N), signed integers Z (a|a:b:c)", 100},
   ...
   {"set_radius",f_set_radius, misc, 1, "set radius of Earth X= 0,2,4,5,6,8 (Code Table 3.2)", -1},
   ...
   {"set_ts_dates",f_set_ts_dates, misc, 3, "changes date code for time series X=YYYYMMDDHH(mmss) Y=dtime Z=block size", 100},
   ...
   {"g2clib",f_g2clib, setup, 1, "X=0/1 use g2clib for decoding grib", 100},
