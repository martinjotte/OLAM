...
real :: rcloud,dummy,temp,rvls,exner  &
   ,uv01dx,uv01dy,uv01dz,uv01dr,raxis,rhovs, add
...
! Fill arrays prior to iteration
   ...
   add = 10.e-6 * max(0.,1000. - sqrt(xew(iw)**2 + yew(iw)**2))
!   add = 10.e-6 * max(0.,1000. - yew(iw))
   ...
   do k = 1,mza
      ...
      elseif (level == 1) then
         sh_w(k,iw) = rt01d(k) 
         sh_v(k,iw) = rt01d(k)
         sh_w(k,iw) = rt01d(k) + add
         sh_v(k,iw) = rt01d(k) + add
      else
         sh_w(k,iw) = rt01d(k)
         sh_v(k,iw) = rt01d(k)
      endif
