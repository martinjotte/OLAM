...
! disabled by commenting out lines of code.
  ...
  use hdf5_f2f,    only: fh5_cache_write_ll, fh5_close_cache
...
!----------
  ...
  real, allocatable :: r_lpw_ll             (:) ! lpw water vapor mixing ratio (kg/kg_dryair)
...
! real, allocatable :: rlongup_top_clr_accum_ll (:) ! accum TOA upward l/w rad (J/m^2)
  ...
  real, allocatable :: asfc_airrrv_accum_ll   (:) ! asfc atm rrv accum (kg s/kg)
  real, allocatable :: asfc_cantempk_accum_ll (:) ! asfc canopy air temp accum (K s)
  real, allocatable :: asfc_canrrv_accum_ll   (:) ! asfc canopy air vap mixing ratio accum (kg s/kg_dryair)
...
!----------
  ...
  real, allocatable :: r_ll (:,:) ! water vapor mixing ratio (kg/kg_dryair)
  ...
  real, allocatable :: r_accum_ll (:,:) ! water vapor mixing ratio accum (kg/kg_dryair)
  real, allocatable :: p_accum_ll (:,:) ! air pressure accum (Pa)

  integer       :: k, iw, ilat, ilon, n, kb, ier, np, j, i, iwsfc, jasfc
  integer       :: iland, iv, jv, npoly, klev
  integer       :: ndims, idims(3)
  character(30) :: dimnames(3)

  real :: raxis, area_land_sum
  real :: vx, vy, vz
...
!    allocate( rlongup_top_clr_accum_ll (npts) ) ; rlongup_top_clr_accum_ll  = rmissing
     ...
     allocate( asfc_airrrv_accum_ll   (npts) ) ; asfc_airrrv_accum_ll    = rmissing
     allocate( asfc_cantempk_accum_ll (npts) ) ; asfc_cantempk_accum_ll  = rmissing
     allocate( asfc_canrrv_accum_ll   (npts) ) ; asfc_canrrv_accum_ll    = rmissing
...
!------------------------------------------------------------
! Copy vapor mixing ratio at lowest prognosed model level
...
!------------------------------------------------------------
! Interpolate vapor mixing ratio, its surface value,
! and the surface value of vapor pressure
!------------------------------------------------------------

  if (do3d)  call interp_htw_ll(npts,iws_loc,wts_loc,mza,mza-1,scr2a,w_ll)
  if (do3d)  call interp_htw_ll(npts,iws_loc,wts_loc,mza,mza-1,rr_v,r_ll)
...
!    call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1a,rlongup_top_clr_accum_ll)
     ...
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1a,asfc_vels_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1b,asfc_airtempk_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1c,asfc_airrrv_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1d,asfc_cantempk_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1e,asfc_canrrv_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1f,asfc_skintempk_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1g,asfc_sensflux_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1h,asfc_latflux_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1i,asfc_vapflux_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1j,al_wxferi_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1k,al_wxferp_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1l,al_wxfer1_accum_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1m,al_sfcwater_tot_ll)
     call interp_htw_ll(npts,iws_loc,wts_loc,1,1,scr1n,al_soil_water_tot_ll)
...
! Store parallel HDF5 file mappings for repeated writes

     if (mpi_does_parallel_io) then
        call cache_ll_writes()
     endif
...
! 'ASFC' quantities are area-weighted averages of land & sea cell quantities over a single atmosphere column
        ...
        CALL shdf5_orec_ll(ndims, idims, 'ASFC_AIRRRV_ACCUM_LL' , rvar1=asfc_airrrv_accum_ll, gpoints=lls_loc, &
                           dimnames = dimnames,                                                              &
        ...
        CALL shdf5_orec_ll(ndims, idims, 'ASFC_CANRRV_ACCUM_LL' , rvar1=asfc_canrrv_accum_ll, gpoints=lls_loc,         &
                           dimnames = dimnames,                                                                     &
...
!                      "Accum TOA upward clear-air l/w radiation", "(J/m^2)", rmissing)
     ...
     call tileplot_ll(nlon,nlat,1,1,alon,alat,npts,lls_loc,asfc_vels_accum_ll,204, &
     ...
     call tileplot_ll(nlon,nlat,1,1,alon,alat,npts,lls_loc,asfc_airrrv_accum_ll,204, &
     ...
     call tileplot_ll(nlon,nlat,1,1,alon,alat,npts,lls_loc,asfc_canrrv_accum_ll,204, &
...
contains
  ...
  subroutine cache_ll_writes()
    ...
    if (dosfc .or. doaccum .or. dodifs) then
       ndims   = 2
       dims(1) = nlon
       dims(2) = nlat

       call fh5_cache_write_ll(ndims, dims, 2, herr, fcoords=lls_loc)
    endif
    ...
    if (do3d) then
       ...
       call fh5_cache_write_ll(ndims, dims, 3, herr, fcoords=lls_loc)
    endif
...
!      dims(3) = npress
!
!      call fh5_cache_write_ll(ndims, dims, 4, herr, fcoords=lls_loc)
!   endif

  end subroutine cache_ll_writes
