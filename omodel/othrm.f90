...
do k = lpw(iw),mza
   ...
   do iterate = 1,20
      ...
      temp = 0.7 * temp  &
           + 0.3 * t_il * (1. + alvlocp * rr_c(k,iw) / max(temp,253.))

!Bob: Martin's (1.+rr_v(k,iw)) factor in the following, though correct for defining energy 
! per kg of total air mass, was not in original RAMS formulation of "per kg of dry air", and
! is also inconsistent with subroutine wetthrm3 (which follows this one).  Hence, for now, I
! return to the original RAMS formulation in the line above.  May want to revisit this in the
! future.  However, it should be noted that the thil equations (Tripoli and Cotton 1981) were
! formulated for the RAMS mixing ratio/dry air equations, and the OLAM specific humidity/total
! density formulation was slightly inconsistent with this.
