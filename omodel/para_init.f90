...
subroutine para_init()
  ...
  ! APPARENTLY, GRIDFILE_READ DOES NOT NEED TO READ ITAB_M%(IV,IW,IMP),
  ! ITAB_V%(IM,IV,IW,IVP), ITAB_W%(IM,IV,IW,IWP,IWNUD).
  ...
  do iw = 2,nwa
     ...
     ! Check if this ATM grid W point is primary on a remote rank
  ...
  if (isfcl == 1) then
     ...
     do ipass = 1,2
      ...
      !n        if (isubdomain == 1) then
      !n           iw = itabg_w(iw)%iw_myrank
      !n        endif
...
subroutine compute_primary_points()
  ...
  if (ia /= mwsfc_primary) stop "error computing number of primary points4"
  if (ib /= mland_primary) stop "error computing number of primary points4.1"
  if (ic /= mlake_primary) stop "error computing number of primary points4.2"
  if (id /= msea_primary) stop "error computing number of primary points4.3"
  ...
  if (mdomain == 0 .and. nudflag > 0 .and. nudnxp > 0) then
     ...
     if (ia /= mwnud_primary) stop "error computing number of primary points8"
  endif
...
do i = 2, mland_primary
   if (iland_globe_primary(i) < iland_globe_primary(i-1)) then
      write(io6,*) 'error: LAND is out of order!!!!'
      stop
   endif
enddo

do i = 2, mlake_primary
   if (ilake_globe_primary(i) < ilake_globe_primary(i-1)) then
      write(io6,*) 'error: LAKE is out of order!!!!'
      stop
   endif
enddo

do i = 2, msea_primary
   if (isea_globe_primary(i) < isea_globe_primary(i-1)) then
      write(io6,*) 'error: SEA is out of order!!!!'
      stop
   endif
enddo
...
do i = 2, mwsfc
   if (itab_wsfc(i)%iwglobe < itab_wsfc(i-1)%iwglobe) then
      write(io6,*) 'error: WSFCGLOBE is out of order!!!!'
      stop
   endif
enddo

do i = 2, mland
   if (itab_land(i)%iwglobe < itab_land(i-1)%iwglobe) then
      write(io6,*) 'error: LANDGLOBE is out of order!!!!'
      stop
   endif
enddo

do i = 2, mlake
   if (itab_lake(i)%iwglobe < itab_lake(i-1)%iwglobe) then
      write(io6,*) 'error: LAKEGLOBE is out of order!!!!'
      stop
   endif
enddo

do i = 2, msea
   if (itab_sea(i)%iwglobe < itab_sea(i-1)%iwglobe) then
      write(io6,*) 'error: SEAGLOBE is out of order!!!!'
      stop
   endif
enddo
