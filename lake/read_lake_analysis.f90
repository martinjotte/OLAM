subroutine read_lake_analysis()
  ...
  real               :: grx, gry, laketemp
  integer            :: nio, njo, ilake, iwsfc
  logical            :: exists, has_laketemp
...
! Process selected analysis file
  ...
  if (.not. exists) then
     ...
     stop 'stop: no analysis file for sst/seaice file'
  endif
  ...
  if (ndims > 0) then
     ...
     call prfill(nprx, npry, a2d, wtemp, gdatdy, xswlat, ipoffset, inproj)
  ...
  !$omp parallel do private(iwsfc,gry,grx,ilat,laketemp)
  do ilake = 2, mlake
     iwsfc = ilake + omlake

     ! Fractional x/y indices in pressure data arrays at current iwsfc location
     ...
     call gdtost(wtemp, nprx+4, npry+4, grx, gry, laketemp)
