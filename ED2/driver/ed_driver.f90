...
! known as master.                                                                         !
!------------------------------------------------------------------------------------------!
subroutine ed_driver(init_type)
   ...
   use mem_polygons, only: ed_reg_latmin, ed_reg_latmax, ed_reg_lonmin, ed_reg_lonmax
   ...
   ! Map IWL index to polygon index
   do ifm = 1, ngrids
      ipoly = 0
      do iwl = 2, mwl
         if(land%glatw(iwl) > ed_reg_latmin(ifm) .and.  &
              land%glatw(iwl) < ed_reg_latmax(ifm) .and. &
             land%glonw(iwl) > ed_reg_lonmin(ifm) .and.  &
             land%glonw(iwl) < ed_reg_lonmax(ifm))then
            ipoly = ipoly + 1
            edgrid_g(ifm)%iwl(ipoly) = iwl
            land%ed_ifm(iwl) = ifm
            land%ed_ipy(iwl) = ipoly
         endif
      enddo
   enddo
...
!      call update_met_drivers(edgrid_g(ifm))
      call olam2ed_state_init(edgrid_g(ifm))
   end do
...
!   end if
   ...
   print*,'bad ed_driver init_type',init_type
   stop
   endif
