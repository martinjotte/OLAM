...
subroutine update_site_derived_props(cpoly,census_flag,isi)
   ...
   use ed_state_vars , only : polygontype  & ! structure
                            ...
                            , patchtype    ! ! structure
   use allometry     , only : ed_biomass   ! ! function
   use ed_consts_coms   , only : pio4         ! ! intent(in)
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
...
!------------------------------------------------------------------------------------------!
subroutine ed_read_soil_moist_temp(cgrid,igr)
   ...
   use soil_coms     , only : soilstate_db & ! intent(in)
                            , soil         & ! intent(in)
                            , slz          ! ! intent(in)
   use ed_consts_coms   , only : cliqvlme     & ! intent(in)
                            , cicevlme     & ! intent(in)
                            , t3ple        & ! intent(in)
   ...
   if (.not.l1) then
      ...
      call fatal_error('Soil database '//trim(soilstate_db)//' not found!'                 &
                     &,'ed_read_soil_moist_temp','update_derived_props.f90')
   end if
...
end subroutine ed_read_soil_moist_temp
!==========================================================================================!
!==========================================================================================!
