...
subroutine compute_budget(csite,lsl,pcpg,qpcpg,ipa,wcurr_loss2atm,ecurr_loss2atm           &
   ...
   use ed_consts_coms  , only : umol_2_kgC         & ! intent(in)
                           , day_sec            & ! intent(in)
                           , rdry               & ! intent(in)
                           , cp                 & ! intent(in)
                           , mmdryi             & ! intent(in)
...
real function compute_water_storage(csite, lsl,ipa)
   ...
   use ed_consts_coms    , only : wdns                  ! ! intent(in)
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   type(sitetype) , target     :: csite
   integer        , intent(in) :: ipa
...
real function compute_energy_storage(csite, lsl, ipa)
   ...
   use grid_coms            , only : nzg                   ! ! intent(in)
   use soil_coms            , only : dslz                  ! ! intent(in)
   use ed_consts_coms          , only : cp                    & ! intent(in)
                                   , cliq                  & ! intent(in)
                                   , cice                  & ! intent(in)
                                   , alvl                  & ! intent(in)
                                   , alli                  & ! intent(in)
...
real function compute_co2_storage(csite,ipa)
   use ed_state_vars  , only : sitetype              ! ! structure
   use ed_consts_coms    , only : mmdryi                ! ! intent(in)
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   type(sitetype)        , target      :: csite
   integer               , intent(in)  :: ipa
