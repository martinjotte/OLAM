...
module sp_therm_lib
   ...
   real function rslf(pres,temp)
      use ed_consts_coms, only : ep,toodry
      implicit none
   ...
   real function rslif(pres,temp,useice)
      use ed_consts_coms, only: t3ple,ep
      implicit none
      real   , intent(in)           :: pres,temp
      logical, intent(in), optional :: useice
      real                          :: esz
   ...
   real function tslif(pvap,useice)
      use ed_consts_coms, only: es3ple,alvl,alvi
   ...
   real function ptrh2rvapil(relh,pres,temp,useice)
      use ed_consts_coms, only: ep,toodry,t3ple
   ...
   real function idealdenssh(pres,temp,qvpr,qtot)
      use ed_consts_coms, only : rdry & ! intent(in)
                            , epi  ! ! intent(in)
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      real, intent(in)           :: pres ! Pressure                                 [   Pa]
   ...
   real function reducedpress(pres,thetaref,shvref,zref,thetacan,shvcan,zcan)
      use ed_consts_coms, only : epim1    & ! intent(in)
                            , p00k     & ! intent(in)
                            , rocp     & ! intent(in)
                            , cpor     & ! intent(in)
                            , cp       & ! intent(in)
   ...
   real function ptqz2enthalpy(pres,temp,qvpr,zref)
      use ed_consts_coms, only : ep       & ! intent(in)
                            , grav     & ! intent(in)
                            , t3ple    & ! intent(in)
                            , eta3ple  & ! intent(in)
                            , cimcp    & ! intent(in)
   ...
   real(kind=4) function thrhsh2temp(theta,dens,qvpr)
      use ed_consts_coms, only : cpocv  & ! intent(in)
                            , p00i   & ! intent(in)
                            , rdry   & ! intent(in)
                            , epim1  & ! intent(in)
                            , rocv   ! ! intent(in)
   ...
   real function theta_iceliq(exner,temp,rliq,rice)
      use ed_consts_coms, only: alvl, alvi, cp, ttripoli, htripoli, htripolii
   ...
   real function dthetail_dt(condconst,thil,exner,pres,temp,rliq,ricein)
      use ed_consts_coms, only: alvl, alvi, cp, ttripoli,htripoli,htripolii,t3ple
   ...
   real function thil2temp(thil,exner,pres,rliq,rice,t1stguess)
      use ed_consts_coms, only: cp, cpi, alvl, alvi, t00, t3ple, ttripoli,htripolii,cpi4
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real function dtempdrs(exner,thil,temp,rliq,rice,rconmin)
      use ed_consts_coms, only: alvl, alvi, cp, cpi, ttripoli, htripolii
   ...
   real function dthil_sedimentation(thil,theta,temp,rold,rnew,qrold,qrnew)
      use ed_consts_coms, only: ttripoli,cp,alvi,alvl
   ...
   real function thetaeiv(thil,pres,temp,rvap,rtot,iflg,useice)
      use ed_consts_coms, only : alvl,alvi,cp,ep,p00,rocp,ttripoli,t3ple
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real function dthetaeiv_dtlcl(theiv,tlcl,rtot,eslcl,useice)
      use ed_consts_coms, only : rocp,aklv,ttripoli
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real function thetaeivs(thil,temp,rsat,rliq,rice)
      use ed_consts_coms, only : aklv, ttripoli
      implicit none
   ...
   real function dthetaeivs_dt(theivs,temp,pres,rsat,useice)
      use ed_consts_coms, only : aklv,alvl,ttripoli,htripolii
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real function thetaeiv2thil(theiv,pres,rtot,useice)
      use ed_consts_coms, only : alvl,cp,ep,p00,rocp,ttripoli,t3ple,t00
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      real   , intent(in)           :: theiv     ! Ice vapour equiv. pot. temp.    [     K]
      real   , intent(in)           :: pres      ! Pressure                        [    Pa]
   ...
   subroutine thetaeivs2temp(theivs,pres,theta,temp,rsat,useice)
      use ed_consts_coms, only : alvl,cp,ep,p00,rocp,ttripoli,t00
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine lcl_il(thil,pres,temp,rtot,rvap,tlcl,plcl,dzlcl,iflg,useice)
      use ed_consts_coms, only: cpog,alvl,alvi,cp,ep,p00,rocp,ttripoli,t3ple,t00,rdry
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine qtk(q,tempk,fracliq)
      use ed_consts_coms, only: cliqi,cicei,allii,t3ple,qicet3,qliqt3,tsupercool
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine qwtk(qw,w,dryhcap,tempk,fracliq)
      use ed_consts_coms, only: cliqi,cliq,cicei,cice,allii,alli,t3ple,tsupercool
      implicit none
      !----- Arguments --------------------------------------------------------------------!
