...
!==========================================================================================!
module dp_therm_lib
   use sp_therm_lib, only : toler4     => toler     & ! intent(in)
   ...
   real(kind=8) function eslif8(temp,useice)
      use ed_consts_coms, only: t3ple8
      implicit none
      real(kind=8), intent(in)           :: temp
   ...
   real(kind=8) function rslf8(pres,temp)
      use ed_consts_coms, only : ep8,toodry8
      implicit none
   ...
   real(kind=8) function rslif8(pres,temp,useice)
      use ed_consts_coms, only: t3ple8,ep8
      implicit none
      real(kind=8), intent(in)           :: pres,temp
      logical     , intent(in), optional :: useice
      real(kind=8)                       :: esz
   ...
   real(kind=8) function tslif8(pvap,useice)
      use ed_consts_coms, only: es3ple8
   ...
   real(kind=8) function ptrh2rvapil8(relh,pres,temp,useice)
      use ed_consts_coms, only: ep8,toodry8,t3ple8
   ...
   real(kind=8) function idealdenssh8(pres,temp,qvpr,qtot)
      use ed_consts_coms, only : rdry8 & ! intent(in)
                            , epi8  ! ! intent(in)
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      real(kind=8), intent(in)           :: pres ! Pressure                         [   Pa]
   ...
   real(kind=8) function reducedpress8(pres,thetaref,shvref,zref,thetacan,shvcan,zcan)
      use ed_consts_coms, only : epim18    & ! intent(in)
                            , p00k8     & ! intent(in)
                            , rocp8     & ! intent(in)
                            , cpor8     & ! intent(in)
                            , cp8       & ! intent(in)
   ...
   real(kind=8) function ptqz2enthalpy8(pres,temp,qvpr,zref)
      use ed_consts_coms, only : ep8       & ! intent(in)
                            , grav8     & ! intent(in)
                            , t3ple8    & ! intent(in)
                            , eta3ple8  & ! intent(in)
                            , cimcp8    & ! intent(in)
   ...
   real(kind=8) function thrhsh2temp8(theta,dens,qvpr)
      use ed_consts_coms, only : cpocv8  & ! intent(in)
                            , p00i8   & ! intent(in)
                            , rdry8   & ! intent(in)
                            , epim18  & ! intent(in)
                            , rocv8   ! ! intent(in)
   ...
   real(kind=8) function theta_iceliq8(exner,temp,rliq,rice)
      use ed_consts_coms, only: alvl8, alvi8, cp8, ttripoli8, htripoli8, htripolii8
   ...
   real(kind=8) function dthetail_dt8(condconst,thil,exner,pres,temp,rliq,ricein)
      use ed_consts_coms, only: alvl8, alvi8, cp8, ttripoli8,htripoli8,htripolii8,t3ple8
   ...
   real(kind=8) function thil2temp8(thil,exner,pres,rliq,rice,t1stguess)
      use ed_consts_coms, only : cp8        & ! intent(in)
                            , cpi8       & ! intent(in)
                            , alvl8      & ! intent(in)
                            , alvi8      & ! intent(in)
                            , t008       & ! intent(in)
   ...
   real(kind=8) function dtempdrs8(exner,thil,temp,rliq,rice,rconmin)
      use ed_consts_coms, only: alvl8, alvi8, cp8, cpi8, ttripoli8, htripolii8
   ...
   real(kind=8) function dthil_sedimentation8(thil,theta,temp,rold,rnew,qrold,qrnew)
      use ed_consts_coms, only: ttripoli8,cp8,alvi8,alvl8
   ...
   real(kind=8) function thetaeiv8(thil,pres,temp,rvap,rtot,useice)
      use ed_consts_coms, only : alvl8,alvi8,cp8,ep8,p008,rocp8,ttripoli8,t3ple8
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real(kind=8) function dthetaeiv_dtlcl8(theiv,tlcl,rtot,eslcl,useice)
      use ed_consts_coms, only : rocp8,aklv8,ttripoli8
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real(kind=8) function thetaeivs8(thil,temp,rsat,rliq,rice)
      use ed_consts_coms, only : aklv8, ttripoli8
      implicit none
      !----- Arguments. -------------------------------------------------------------------!
      real(kind=8), intent(in)   :: thil     ! Ice-liquid water potential temp.    [     K]
      real(kind=8), intent(in)   :: temp     ! Temperature                         [     K]
   ...
   real(kind=8) function dthetaeivs_dt8(theivs,temp,pres,rsat,useice)
      use ed_consts_coms, only : aklv8,alvl8,ttripoli8,htripolii8
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   real(kind=8) function thetaeiv2thil8(theiv,pres,rtot,useice)
      use ed_consts_coms, only : alvl8,cp8,ep8,p008,rocp8,ttripoli8,t3ple8,t008
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine thetaeivs2temp8(theivs,pres,theta,temp,rsat,useice)
      use ed_consts_coms, only : alvl8,cp8,ep8,p008,rocp8,ttripoli8,t008
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine lcl_il8(thil,pres,temp,rtot,rvpr,tlcl,plcl,dzlcl,useice)
      use ed_consts_coms, only: cpog8, alvl8,alvi8,cp8,ep8,p008,rocp8,ttripoli8,t3ple8,t008
      implicit none
      !----- Arguments --------------------------------------------------------------------!
      real(kind=8), intent(in)            :: thil   ! Ice liquid pot. temp. (*)   [      K]
      real(kind=8), intent(in)            :: pres   ! Pressure                    [     Pa]
   ...
   subroutine qtk8(q,tempk,fracliq)
      use ed_consts_coms, only: cliqi8,cicei8,allii8,t3ple8,qicet38,qliqt38,tsupercool8
      implicit none
      !----- Arguments --------------------------------------------------------------------!
   ...
   subroutine qwtk8(qw,w,dryhcap,tempk,fracliq)
      use ed_consts_coms, only: cliqi8,cliq8,cicei8,cice8,allii8,alli8,t3ple8,tsupercool8
      implicit none
      !----- Arguments --------------------------------------------------------------------!
