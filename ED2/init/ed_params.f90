...
subroutine init_ed_misc_coms
   use ed_max_dims  , only : n_pft               & ! intent(in)
                           , n_dbh               & ! intent(in)
                           , n_age               ! ! intent(in)
   use ed_consts_coms  , only : erad                & ! intent(in)
                           , pio180              ! ! intent(in)
...
subroutine init_can_rad_params()
   ...
   use ed_consts_coms           , only : pio180                      ! ! intent(out)
   implicit none
...
subroutine init_can_air_params()
   use ed_consts_coms    , only : onethird              & ! intent(in)
                             , twothirds             & ! intent(in)
                             , vonk                  ! ! intent(in)
...
subroutine init_pft_photo_params()
   ...
   use ed_consts_coms    , only : t00                     & ! intent(in)
                             , twothirds               & ! intent(in)
...
subroutine init_decomp_params()
   use ed_consts_coms , only : yr_day                      ! ! intent(in)
...
subroutine init_pft_resp_params()
   ...
   use ed_consts_coms    , only : onesixth                  & ! intent(in)
                             , onethird                  ! ! intent(in)
   implicit none
...
subroutine init_pft_alloc_params()
   ...
   use allometry   , only : h2dbh                 & ! function
                          , dbh2bd                ! ! function
   use ed_consts_coms , only : twothirds             & ! intent(in)
                          , pi1                   ! ! intent(in)
   use ed_max_dims , only : n_pft                 & ! intent(in)
                          , str_len               ! ! intent(in)
...
subroutine init_pft_leaf_params()
   ...
   use pft_coms       , only : phenology            & ! intent(out)
                             ...
                             , delta_c              ! ! intent(out)
   use ed_consts_coms    , only : t3ple                ! ! intent(out) 
...
subroutine init_physiology_params()
   ...
   use ed_consts_coms    , only : umol_2_mol          & ! intent(in)
                             , t00                 & ! intent(in)
                             , mmdoc               & ! intent(in)
                             , mmcod               & ! intent(in)
                             , mmo2                & ! intent(in)
...
subroutine init_soil_coms
   ...
   use ed_consts_coms    , only : grav                  & ! intent(in)
                             , hr_sec                & ! intent(in)
                             , day_sec               ! ! intent(in)
...
subroutine init_ff_coms
   ...
   use ed_consts_coms        , only : onethird           & ! intent(out)
                                 , twothirds          ! ! intent(in)
   implicit none
   !----- Local variables. ----------------------------------------------------------------!
   real              :: exp_patfus
...
subroutine init_rk4_params()
   ...
   use met_driver_coms, only : prss_min               & ! intent(in)
                             , prss_max               ! ! intent(in)
   use ed_consts_coms    , only : wdnsi8                 ! ! intent(in)
