...
subroutine normalize_ed_daily_vars(cgrid,timefac1)
   ...
   use ed_misc_coms  , only : imoutput      & ! intent(in)
                            ...
                            , ddbhi         & ! intent(in)
                            , dagei         ! ! intent(in)
   use ed_consts_coms   , only : umols_2_kgCyr ! ! intent(in)
   implicit none
   !----- Arguments. ----------------------------------------------------------------------!
   type(edtype)                                  , target     :: cgrid
...
subroutine normalize_ed_daily_output_vars(cgrid)
   ...
   use ed_max_dims          , only : n_pft         & ! intent(in)
                                   ...
                                   , n_age         & ! intent(in)
                                   , n_dist_types  ! ! intent(in)
   use ed_consts_coms          , only : cpi           & ! intent(in)
                                   , alvl          & ! intent(in)
                                   , day_sec       & ! intent(in)
                                   , umols_2_kgCyr & ! intent(in)
                                   , yr_day        & ! intent(in)
...
subroutine integrate_ed_monthly_output_vars(cgrid)
   ...
   use ed_max_dims  , only : n_dbh         & ! intent(in)
                           ...
                           , n_dist_types  & ! intent(in)
                           , n_mort        ! ! intent(in)
   use ed_consts_coms ,  only : yr_day
   implicit none
   !----- Argument. -----------------------------------------------------------------------!
   type(edtype)      , target  :: cgrid
   !----- Local variables. ----------------------------------------------------------------!
...
subroutine normalize_ed_monthly_output_vars(cgrid)
   ...
   use ed_max_dims          , only : n_pft         & ! intent(in)
                                   ...
                                   , n_dist_types  & ! intent(in)
                                   , n_mort        ! ! intent(in)
   use ed_consts_coms          , only : p00i          & ! intent(in)
                                   , rocp          & ! intent(in)
                                   , pio4          & ! intent(in)
                                   ...
                                   , yr_day        ! ! intent(in)
   use pft_coms             , only : init_density  ! ! intent(in)
   use sp_therm_lib            , only : idealdenssh   & ! function
                                   , qwtk          ! ! function
   ...
   polyloop: do ipy=1,cgrid%npolygons
      ...
      if (iqoutput > 0) then
         do t=1,ndcycle
            ...
            if(cgrid%qmean_can_prss(t,ipy) > 0.0)then
