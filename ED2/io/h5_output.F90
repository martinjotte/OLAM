...
subroutine h5_output(vtype)
   ...
   use grid_coms    , only : ngrids                & ! intent(in)
                           , time                  ! ! intent(in)
   use ed_hdf5_coms    , only : chnkdims              & ! intent(in)
                           , chnkoffs              & ! intent(in)
                           , cnt                   & ! intent(in)
                           , stride                & ! intent(in)
                           , globdims              ! ! intent(in)
   ...
   gridloop: do ngr=1,ngrids
      ...
      if (gdpy(mynum,ngr)>0) then
         ...
         case ('DAIL')
            ...
            outhour = 0
            !----- Make the file name. ----------------------------------------------------!
            call ed_makefnam(anamel,ffilout,zero,outyear,outmonth,outdate,outhour,vnam,cgrid  &
                         ,'h5 ')
         ...
         case ('OPTI')
            ...
            call ed_makefnam(anamel,ffilout,zero,outyear,0,0,0,vnam,cgrid,'h5 ')
         ...
         case ('HIST','CONT')
            ...
            call ed_makefnam(anamel,sfilout,time,iyeara,imontha,idatea,itimea*100,vnam,cgrid  &
                         ,'h5 ')
            outyear  = iyeara
            outmonth = imontha
            outdate  = idatea
         ...
         case default
            if (nrec_fast == 1) then
               ...
               call ed_makefnam(anamel,ffilout,time,iyeara,imontha,idatea,itimea*100,vnam     &
                            ,cgrid,'h5 ')
            else   
               ...
               call ed_makefnam(anamel,ffilout,zero,out_time_fast%year,out_time_fast%month    &
   ...
   case ('HIST','CONT')
      ...
      if (nnodetot /= 1 .and. mynum == nnodetot) then
         ...
         call ed_makefnam(anamel,sfilout,time,iyeara,imontha,idatea,itimea*100,vnam,'g00'     &
                      ,'cmp')
