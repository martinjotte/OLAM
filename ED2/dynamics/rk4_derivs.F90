...
subroutine leaftw_derivs(mzg,mzs,initp,dinitp,csite,ipa)
   use ed_max_dims          , only : nzgmax                & ! intent(in)
                                   , nzsmax                ! ! intent(in)
   use ed_consts_coms          , only : alvl8                 & ! intent(in)
                                   , cliqvlme8             & ! intent(in)
   ...
   use ed_state_vars        , only : sitetype              & ! structure
                                   , patchtype             & ! structure
                                   , polygontype
   use dp_therm_lib           , only : qtk8                  ! ! subroutine
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
   type(rk4patchtype)  , target     :: initp            ! RK4 structure, intermediate step
   type(rk4patchtype)  , target     :: dinitp           ! RK4 structure, derivatives
...
subroutine canopy_derivs_two(mzg,initp,dinitp,csite,ipa,hflxgc,wflxgc,qwflxgc,dewgndflx    &
   ...
   use soil_coms             , only : soil8                & ! intent(in)
                                    , dslzi8               & ! intent(in)
                                    , dewmax               ! ! intent(in)
   use dp_therm_lib            , only : rslif8               ! ! function
   use ed_misc_coms          , only : dtlsm                & ! intent(in)
                                    , fast_diagnostics     ! ! intent(in)
   ...
   if (any_resolvable) then
      ...
      taii = 1.d0/taii
!print*,'pcpg'
!print*,rk4site%pcpg,closedcan_frac,leaf_intercept
      !------------------------------------------------------------------------------------!
   ...
   cohortloop: do ico = 1,cpatch%ncohorts
      ...
      if (initp%leaf_resolvable(ico)) then
         ...
         !---------------------------------------------------------------------------------!
!print*,'leaf derivs',ico
!print*,effarea_heat,initp%lai(ico),initp%leaf_gbh(ico),initp%leaf_temp(ico),initp%can_temp
...
!print*,'intcept'
!print*,intercepted_max, initp%lai(ico), taii
...
!print*,'leaf water',ico
!print*,leaf_intercepted, wflxlc, wshed
...
!print*,'cohort',ico
!print*,initp%rshort_l(ico),initp%rlong_l(ico),hflxlc,qwflxlc,qwshed,qtransp,leaf_qintercepted
