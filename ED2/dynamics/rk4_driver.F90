...
iwl = cgrid%iwl(ipy)
         siteloop: do isi = 1,cpoly%nsites
            ...
            cmet  => cpoly%met(isi)
!print*,'pcpg in driver'
!print*,cmet%pcpg
            patchloop: do ipa = 1,csite%npatches
               cpatch => csite%patch(ipa)

	       ! Format soil_energy and sfcwater_energy for ED.

	       call format_soil_energy_for_ed(csite,ipa,cpoly%ntext_soil(1:nzg,isi))
...
!               end if
!               cmet%vels = land%vels(cgrid%iwl(ipy))
               ...
               !---------------------------------------------------------------------------!
               integration_buff%initp%can_prss = cmet%prss
               integration_buff%initp%can_exner = cmet%exner
...
!===========
	       ...
	       ! Format soil_energy and sfcwater_energy for OLAM.
...
print*,'finished rk4'
   ...
   subroutine integrate_patch_rk4(csite,initp,ipa,wcurr_loss2atm,ecurr_loss2atm            &
      ...
      use ed_consts_coms     , only : vonk8                & ! intent(in)
                                 , cp8                  & ! intent(in)
                                 , cpi8                 ! ! intent(in)
      use rk4_coms        , only : integration_vars     & ! structure
                                 , rk4patchtype         & ! structure
   ...
   subroutine initp2modelp(hdid,initp,csite,ipa,wbudget_loss2atm,ebudget_loss2atm          &
      ...
      use ed_state_vars        , only : sitetype             & ! structure
                                      , patchtype            & ! structure
                                      , edgrid_g             ! ! structure
      use ed_consts_coms          , only : day_sec              & ! intent(in)
                                      , t3ple8               ! ! intent(in)
      ...
      use grid_coms            , only : nzg                  & ! intent(in)
                                      , nzs                  ! ! intent(in)
      use sp_therm_lib            , only : qwtk                 & ! subroutine
                                      , rslif                ! ! function
