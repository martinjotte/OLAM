...
subroutine copy_patch_init(sourcesite,ipa,targetp)
   ...
   use ed_consts_coms           , only : cpi8                   & ! intent(in)
                                    , ep8                    & ! intent(in)
                                    , cp8                    & ! intent(in)
                                    , epim18                 & ! intent(in)
                                    , alvl8                  & ! intent(in)
   ...
   use dp_therm_lib            , only : qwtk8                  & ! subroutine
                                    , thetaeiv8              & ! function
...
subroutine copy_patch_init_carbon(sourcesite,ipa,targetp)
   use ed_state_vars        , only : sitetype              & ! structure
                                   , patchtype             ! ! structure
   use ed_consts_coms          , only : day_sec8              & ! intent(in)
...
subroutine adjust_sfcw_properties(nzg,nzs,initp,hdid,csite,ipa)
   ...
   use soil_coms     , only : soil8                 & ! intent(in)
                            ...
                            , thick                 & ! intent(in)
                            , thicknet              ! ! intent(in)
   use ed_consts_coms   , only : cice8                 & ! intent(in)
                            , cliq8                 & ! intent(in)
                            , t3ple8                & ! intent(in)
                            , wdns8                 & ! intent(in)
                            , wdnsi8                & ! intent(in)
...
subroutine print_rk4patch(y,csite,ipa)
   ...
   use grid_coms             , only : nzg                   & ! intent(in)
                                    , nzs                   ! ! intent(in)
   use ed_misc_coms          , only : current_time          ! ! intent(in)
   use ed_consts_coms           , only : pio1808               ! ! intent(in)
   use dp_therm_lib            , only : qtk8                  & ! subroutine
                                    , qwtk8                 ! ! subroutine
   implicit none
   !----- Arguments -----------------------------------------------------------------------!
...
subroutine print_rk4_state(initp,fluxp,csite,ipa,elapsed,hdid)
   use ed_consts_coms  , only : t3ple8        ! ! intent(in)
   ...
   use dp_therm_lib   , only : qwtk8         ! ! sub-routine
   use soil_coms    , only : soil8         ! ! intent(in)
   implicit none
   !----- Arguments. ----------------------------------------------------------------------!
   type(rk4patchtype)    , target     :: initp
