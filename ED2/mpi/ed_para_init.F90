...
subroutine ed_node_decomp(init,standalone,masterworks)
   ...
   use mem_polygons, only: ed_reg_latmax, ed_reg_latmin, ed_reg_lonmin, ed_reg_lonmax
   implicit none
   !----- Arguments. ----------------------------------------------------------------------!
   ...
   if(grid_type /= 2)then
      ...
      do ngr = 1, ngrids
        do iwl = 2, mwl
          if(land%glatw(iwl) > ed_reg_latmin(ngr) .and. land%glatw(iwl) < ed_reg_latmax(ngr) .and. &
             land%glonw(iwl) > ed_reg_lonmin(ngr) .and. land%glonw(iwl) < ed_reg_lonmax(ngr))then
             mmyp(ngr) = mmyp(ngr) + 1
             land%ed_flag(iwl) = 1
          endif
        enddo
        if(mmyp(ngr) > 0)then
          call ed_nullify_work(work_e(ngr))
          call ed_alloc_work(work_e(ngr),mmxp(ngr),mmyp(ngr),maxsite)
        endif
      enddo
   endif
...
subroutine get_grid
   ...
   use leaf_coms, only: mwl
   use mem_leaf, only: land
   use mem_polygons, only: ed_reg_latmax, ed_reg_lonmax
   implicit none
   ...
   case (2) !----- OLAM land grid.-----------------------------!
      do ifm = 1, n_ed_region
        my_count = 0
        do iwl = 2, mwl
          if(land%glatw(iwl) > ed_reg_latmin(ifm) .and. land%glatw(iwl) < ed_reg_latmax(ifm) .and. &
             land%glonw(iwl) > ed_reg_lonmin(ifm) .and. land%glonw(iwl) < ed_reg_lonmax(ifm))then
             my_count = my_count + 1
             work_e(ifm)%glon(1,my_count) = land%glonw(iwl)
             work_e(ifm)%glat(1,my_count) = land%glatw(iwl)
          endif
        enddo
      enddo
      ...
      do j = 1,nyp
         do i=1,nxp
            ...
            case(2)
               ...
               lon_list(2:3,ipy) = lon_list(1,ipy)
               lat_list(2:3,ipy) = lat_list(1,ipy)
...
subroutine ed_load_work_from_history()
   ...
   use ed_work_vars, only : work_v              & ! intent(inout)
                          ...
                          , npolys_run          ! ! intent(out)
   use mem_polygons, only : n_ed_region         ! ! intent(in)
   use ed_hdf5_coms   , only : file_id             & ! intent(inout)
   ...
   gridloop: do ifm = 1, n_ed_region
      ...
      if (trim(runtype) == 'HISTORY') then
         ...
         call ed_makefnam(hnamel,sfilin(1),dbletime,current_time%year,current_time%month      &
