Module nuclei_coms
  ...
  real,    parameter :: rhoa = 1.25e-3, grav = 981., fact = 1.153e-16

  real :: rad(nbin) = & ! dust particle radius [cm] for each bin
          [ 0.150e-4, 0.265e-4, 0.471e-4, 0.838e-4, 1.50e-4, 2.65e-4, 4.71e-4 ]

  real :: sp (nbin) = & ! relative abundance [fractional mass] of each bin
          [ 0.0009, 0.0081, 0.0234, 0.0676, 0.3000, 0.3000, 0.3000 ]

  real :: den(nbin) = & ! density [g/cm^3] of particles in each bin
          [ 2.50, 2.50,  2.50, 2.50, 2.65, 2.65, 2.65 ]
  ...
  integer :: jsfcinx(0:nvtyp) = &
             [ 0,0,12,8,1,3,4,2,6,6,8,9,10,10,10,7,7,11,6,15,1,6 ]

  real :: acoll(nddc) = & ! radius of collectors
          [ 2.0,5.0,2.0,5.0,5.0,2.0,2.0,0.0,0.0,10.0,10.0,0.0,0.0,0.0,10.0 ]

  real :: alpha(nddc) = & ! property of vegtype
          [  1.0,  0.6,  1.1,  0.8,   0.8,   1.2, 1.2, 50.0, &
            50.0,  1.3,  2.0, 50.0, 100.0, 100.0, 1.5 ]

  real :: ggamma(nddc) = &
          [ 0.56, 0.58, 0.56, 0.56, 0.56, 0.54, 0.54, 0.54, &
...
!===============================================================================
  ...
  subroutine dust_src_init()
    ...
    do ibin = 1, nbin
       amassi(ibin) = 1. / (den(ibin) * (4./3.) * pi1 * rad(ibin)**3)
