...
subroutine olam_alloc_mpi(mza, mrls)
  ...
  use mem_para,   only: nbytes_int, nbytes_real, nbytes_real8,                  &
                        nrecvs_v, nrecvs_w, nrecvs_m, nrecvs_wnud, nrecvs_wsfc, &
                        nsends_v, nsends_w, nsends_m, nsends_wnud, nsends_wsfc, &
                        recv_v,   recv_w,   recv_m,   recv_wnud,   recv_wsfc,   &
                        send_v,   send_w,   send_m,   send_wnud,   send_wsfc,   &
                        ireqs_v,  ireqs_w,  ireqs_m,  ireqs_wnud,  ireqs_wsfc
...
! Allocate recv_wnud buffers
  ...
  allocate( ireqs_v( nrecvs_v(1) ) )
  allocate( ireqs_w( nrecvs_w(1) ) )
  allocate( ireqs_m( nrecvs_m(1) ) )
  allocate( ireqs_wnud( nrecvs_wnud ) )
...
! Subroutine to perform a parallel MPI send of a "V group" of field variables
  ...
  use mem_para,   only: send_v, recv_v, nsends_v, nrecvs_v, itagv, nbytes_int, ireqs_v
...
subroutine mpi_send_m(mrl, rvara1, rvara2)
  ...
  use mem_para,   only: nrecvs_m, nsends_m, recv_m, send_m, itagm, nbytes_int, ireqs_m
...
subroutine mpi_send_wnud(dvara1, dvara2, dvara3, dvara4, dvara5, dvara6)
  ...
  use mem_para,   only: nrecvs_wnud, nsends_wnud, recv_wnud, send_wnud, &
                        itagwnud, nbytes_int, ireqs_wnud
...
! of field variables
  ...
  use mem_para,   only: recv_v, nrecvs_v, ireqs_v
  ...
  do jtmp = 1,nrecvs_v(mrl)
     ...
     call MPI_Waitany(nrecvs_v(mrl), ireqs_v, jrecv, MPI_STATUS_IGNORE, ierr)
...
! of field variables
  ...
  use mem_para,   only: recv_m, nrecvs_m, ireqs_m
  ...
  do jtmp = 1, nrecvs_m(mrl)
     ...
     call MPI_Waitany(nrecvs_m(mrl), ireqs_m, jrecv, MPI_STATUS_IGNORE, ierr)
...
! of field variables
  ...
  do jtmp = 1, nrecvs_w(mrl)
     ...
     call MPI_Waitany(nrecvs_w(mrl), ireqs_w, jrecv, MPI_STATUS_IGNORE, ierr)
...
! of field variables
  ...
  use mem_para,    only: recv_wnud, nrecvs_wnud, ireqs_wnud
  use mem_grid,    only: mza
  ...
  do jtmp = 1, nrecvs_wnud
     ...
     call MPI_Waitany(nrecvs_wnud, ireqs_wnud, jrecv, MPI_STATUS_IGNORE, ierr)
