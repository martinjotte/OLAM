...
write(io6,*) 'emis_database_read ', iemisfile, trim(emis_files(iemisfile))
...
do n = 2, nvars3d_emis+1
   ...
   rcode = nf90_get_var( ncid, n, data, start=(/1,1,1,1/), &
                         count=(/ncols_emis,nrows_emis,nlays_emis,1/) )

   ! divide by cell area to get emissions per m^2
