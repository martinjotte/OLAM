...
C-------------------------------------------------------------------------------
     ...
     &         + landflux(ilf)%sfluxr * .61 * theta(kw,iw)
         ...
         tempcr = land%can_temp(iwl)
         rh_air = 100.0 * land%can_shv(iwl) * rho(kw,iw) / rhovsil( land%can_temp(iwl) - 273.15 )
     ...
     &        ( glon .GE. -120.0 ) .AND. ( glon .LE. -70.0 ) ) THEN
         ...
         depvel_gas_land( ilf,: ) = 0.0  ! initialize for this time period
...
!           IF ( depspc( l ) .EQ. 'SULF' ) THEN  ! Sulfate (SULF)
         ...
         ! Sulfate calculation follows Wesely (1985), Eqn. 11.
               ...
               depvel_gas_land( ilf,n ) =  1.0 / ( ra + rbsulf )
...
!     &                + real( ifsnow ) * ( (1.0 - xm) / rsnow + xm / (rsndiff + rgw) )
     ...
     &                    delta * lai / rwetsfc + 1.0 / rgndc )
         ...
         ! Compute dry deposition velocity.
               ...
               depvel_gas_land( ilf,n ) = 1.0 / ( rsurf + rac )
...
! depositional loss.
                     ...
                     depvel_gas_land( ilf,n ) = depvel_gas_land( ilf,n ) + 2.0 * kno2 * zf
...
! Add this to the HONO emissions for now when we apply it in vertical diffusion
                     ...
                     vdemis_gc(kw,iw,ie_hono) = vdemis_gc(kw,iw,ie_hono) + pvd * landflux(ilf)%area * volti(kw,iw)
            ...
            IF ( depvel_gas_land( ilf,n ) .LT. 0.0 .OR.
     &           depvel_gas_land( ilf,n ) .NE. depvel_gas_land( ilf,n ) ) then
      ...
      do j = 1,jseaflux(1)%jend(mrl)
         ...
         wtv0  = seaflux(isf)%sfluxt * (1. + .61 * sh_v(kw,iw))
     &         + seaflux(isf)%sfluxr * .61 * theta(kw,iw)
         ...
         depvel_gas_sea( isf,: ) = 0.0  ! initialize for this time period
...
!           IF ( depspc( l ) .EQ. 'SULF' ) THEN  ! Sulfate (SULF)
         ...
         ! from Hayduk and Laudie
               ...
               rsurf     = scw_pr_23 / ( heff * d3 * ustar )

         ! Compute dry deposition velocity over sea

               rbc = 5.0 / ustarw * scc_pr_23( l )

               depvel_gas_sea( isf,n ) = 1.0 / ( rsurf + raw + rbc )
         ...
         ! Compute dry deposition velocity contribution from seaice
                  ...
                  depvel_gas_sea( isf,n ) = (1.0 - sea%seaicec(iws)) * depvel_gas_sea( isf,n )
     &                                    +        sea%seaicec(iws)  * d_ice
         ...
         ! Check for negative values or NaN's
            ...
            IF ( depvel_gas_sea( isf,n ) .LT. 0.0 .OR.
     &           depvel_gas_sea( isf,n ) .NE. depvel_gas_sea( isf,n ) ) then
