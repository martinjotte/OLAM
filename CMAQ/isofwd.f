...
C *** CALCULATE OBJECTIVE FUNCTION ************************************
C
CCC30    FUNCB3A= ( SO4I*NH4I**2 )/( XK7*(WATER/GAMA(4))**3 )
30    FUNCB3A= MOLAL(5)*MOLAL(3)**2
...
C *** CALCULATE FUNCTION VALUE FOR OUTER LOOP ***************************
C
50    A2      = XK13*(WATER/GAMA(13))**5
      FUNCI3A = MOLAL(5)*MOLAL(6)*MOLAL(3)**3/A2 - ONE
      RETURN
C
...
C *** CALCULATE FUNCTION VALUE FOR OUTER LOOP ***************************
C
20    A2      = XK13*(WATER/GAMA(13))**5
      FUNCI2A = MOLAL(5)*MOLAL(6)*MOLAL(3)**3/A2 - ONE
      RETURN
C
...
C
      ...
      A3 = XK1  *WATER/GAMA(7)*(GAMA(8)/GAMA(7))**2
C
      PSI2 = 0.5*(-(LAMDA+PSI1) + SQRT((LAMDA+PSI1)**2+4.D0*A2))  ! PSI2
      PSI2 = MIN (PSI2, CHI2)
C
      BB   = A3+LAMDA                        ! KAPA
...
C
      DO 10 I=1,NSWEEP
C
C      A1  = XK5 *(WATER/GAMA(2))**3
      A4  = (XK2/XKW)*R*TEMP*(GAMA(10)/GAMA(5))**2
      A5  = XK4 *R*TEMP*(WATER/GAMA(10))**2
      A6  = XK3 *R*TEMP*(WATER/GAMA(11))**2
C      A7  = XK8 *(WATER/GAMA(1))**2
C      A8  = XK9 *(WATER/GAMA(3))**2
...
C         DIAK = MAX(VITA**2 - 4.0D0*GKAMA,ZERO)
...
CC
C      DIAK  = SQRT( (ALFA1)**2 + 4.0D0*ALFA1*X)
