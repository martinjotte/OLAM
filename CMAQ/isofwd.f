...
C
      ...
      REAL(8) :: WI(NCOMP)
      REAL    :: RHI, TEMPI
      REAL(8) :: NAFRI, NO3FRI, REST, REST1, REST2, REST3
      REAL(8) :: CCASO4I, FRSO4I, CAFRI, CCANO32I, CCACL2I, CLFRI, CMGCL2I
...
C
      ...
      IMPLICIT NONE
      REAL(8) :: OMELO, OMEHI, X1, Y1, ABS, X2, Y2, X3, Y3, A20, A30
      REAL    :: DYM
      INTEGER :: I
C
C *** SETUP PARAMETERS ************************************************
...
C
      ...
      INTEGER             :: I
      REAL(8)             :: LAMDA, NH4I, SO4I, A1, A2, A3, ZETA, PSI, OMEG
C
C *** SETUP PARAMETERS ************************************************
C
      FRST   = .TRUE.
...
C
      DO I = 1, NSWEEP
         ...
         A3    = A30 * REAL(WATER,8)**2
C
         LAMDA = PSI * OMEGI / (A1 + OMEGI)
         ZETA  = A3  * OMEG

         SO4I = MAX(PSI-LAMDA,TINY)
         NH4I = MAX( W(3)*A2/(OMEG+A2), 2.D0*SO4I )
C
...
C
      ...
      REAL(8) :: PSI4LO, PSI4HI, P4, A40, A340, A70
...
C
 ...
 50   IF (MOLAL(1).GT.TINY) THEN
      ...
      REAL(8), INTENT(IN) :: P4
      INTEGER :: I
      REAL(8) :: PSI4, A34, A7, PSI3, BB, AHI, NH4I
C
C *** SETUP PARAMETERS ************************************************
C
      FRST   = .TRUE.
...
C *** CALCULATE OBJECTIVE FUNCTION ************************************
C
C     A4     = A40 * (GAMA(10)/GAMA(5))**2
C     FUNCD3 = NH4I - AHI * GNH3 * A4
      FUNCD3 = NH4I * gama(5)**2 - AHI * GNH3 * A40 * gama(10)**2
C
      END FUNCTION FUNCD3
...
C
 50   IF (MOLAL(1).GT.TINY .AND. MOLAL(5).GT.TINY) THEN ! If quadrat.called
      ...
      REAL(8)             :: PSI4, PSI5, BB, CC, DD, A6_A5, A4M, FRNH4
      REAL                :: SMIN, HI, OHI
      INTEGER             :: I
C
C *** SETUP PARAMETERS ************************************************
C
      ...
      GHCL     = MAX(CHI6 - PSI6, ZERO)                 ! Gas HCl
      MOLAL(4) = PSI6                                 ! CLI
C
C *** SOLVE EQUATIONS ; WITH ITERATIONS FOR ACTIVITY COEF. ************
C
...
C
      ...
      GNH3      = MAX(CHI4 - PSI4, ZERO)              ! Gas NH3
      GHNO3     = MAX(CHI5 - PSI5, ZERO)              ! Gas HNO3
...
C
C     A6      = A60 * REAL( WATER/GAMA(11), 8)**2
C     FUNCG5A = HI * PSI6 - GHCL * A6
      FUNCG5A = HI * PSI6 * gama(11)**2 - GHCL * A60 * WATER**2
...
C
 50   IF (MOLAL(1).GT.TINY .AND. MOLAL(5).GT.TINY) THEN
      ...
      IMPLICIT NONE
      REAL(8), INTENT(IN) :: PSI6
      REAL(8)             :: A6_A5, PSI5, A4M, BB, CC, DD, PSI4, FRNH4, CLI
      REAL                :: SMIN, HI, OHI
      INTEGER             :: I
C
C *** SETUP PARAMETERS ************************************************
...
C
      IF (CHI5 > TINY) THEN
         ...
         PSI5 = (CHI5*CLI - A6_A5*CHI8) / (A6_A5 + CLI)
...
C
      ...
      GNH3      = MAX(CHI4 - PSI4, ZERO)
      GHNO3     = MAX(CHI5 - PSI5, ZERO)
...
C *** CALCULATE FUNCTION VALUE FOR OUTER LOOP ***************************
C
C     A64     = A640 * REAL(WATER * GAMA(10) / (GAMA(11) * GAMA(5)), 8)**2
C     FUNCH6A = PSI4 * CLI - GHCL * GNH3 * A64
      FUNCH6A = PSI4 * CLI * (GAMA(11) * GAMA(5))**2
     &        - GHCL * GNH3 * A640 * (WATER * GAMA(10))**2
...
C
 50   IF (MOLAL(1).GT.TINY .AND. MOLAL(5).GT.TINY) THEN
      ...
      IMPLICIT NONE
      REAL(8), INTENT(IN) :: PSI6
      REAL(8)             :: A6_A5, PSI5, A4M, BB, CC, DD, PSI4, FRNH4
      INTEGER             :: I
      REAL                :: HI, OHI, SMIN
C
C *** SETUP PARAMETERS ************************************************
...
C
C     A6     = A60 * REAL( WATER / GAMA(11), 8)**2
C     FUNCO7 = HI * psi6 - GHCL * A6
      FUNCO7 = HI * PSI6 * GAMA(11)**2 - GHCL * A60 * WATER**2
...
C
 50   IF (MOLAL(1).GT.TINY .AND. MOLAL(5).GT.TINY) THEN
      ...
      REAL(8), INTENT(IN) :: PSI6
      REAL                :: SMIN, HI, OHI
      REAL(8)             :: CLI, A6_A5, PSI4, PSI5, A4M, BB, CC, DD, FRNH4
      INTEGER             :: I
C
C *** SETUP PARAMETERS ************************************************
C
...
C
C     A6     = A60 * REAL( WATER / GAMA(11), 8)**2
C     FUNCM8 = MOLAL(1) * MOLAL(4) *  GAMA(11)**2 - GHCL * A6
      FUNCM8 = MOLAL(1) * MOLAL(4) * GAMA(11)**2 - GHCL * A60 * WATER**2
...
C
      ...
      REAL(8) :: A650, A4M0, A60, CLI0, NO3I0
...
C
      ...
      CLI0   = CHI7 + CHI14 + 2.D0*CHI16 + 2.D0*CHI17
      NO3I0  = CHI8 + 2.D0*CHI12 + CHI13 + 2.D0*CHI15
C
C *** INITIAL VALUES FOR BISECTION ************************************
C
      X1 = PSI6LO
...
C
      IF (CHI5 > TINY) THEN
         A6_A5= A650 * GHCL * (GAMA(10) / GAMA(11))**2
         PSI5 = (CHI5*CLI - A6_A5*NO3I0) / (A6_A5 + CLI)
         PSI5 = MIN(MAX(PSI5, TINY),CHI5)
      ELSE
         PSI5 = CHI5
      ENDIF
...
C
C     A6      = A60 * REAL( WATER/GAMA(11), 8)**2
C     FUNCP13 = HI * CLI - GHCL * A6
      FUNCP13 = HI * CLI * GAMA(11)**2 - GHCL * A60 * WATER**2
C
...
C
      ...
      REAL(8) :: A9, BB, DD, LAMDA, SO4A, HSO4A
