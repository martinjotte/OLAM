...
C
     ...
     &                     WT, GAS, AERLIQ, AERSLD, SCASI, OTHER)
      ...
      INTEGER            :: SCASI
      REAL(8)            :: WI(NCOMP), WT(NCOMP), GAS(NGASAQ)
      REAL(8)            :: AERSLD(NSLDS), AERLIQ(NIONS+NGASAQ+2)
      REAL(8)            :: CNTRL(NCTRL), OTHER(NOTHER)
...
C
      IPROB   = NINT(CNTRL(1))
C
C *** AEROSOL STATE IS ALWAYS METASTABLE) *******************************
C
C     METSTBL = NINT(CNTRL(2))
C
C *** SOLVE FOREWARD PROBLEM ********************************************
C
50    IF (IPROB.EQ.0) THEN
         IF (WI(1)+WI(2)+WI(3)+WI(4)+WI(5)+WI(6)+WI(7)+WI(8) .LE. 8.d0*TINY)
     &           THEN                                                 !Everything=0
            CALL INIT1 (WI, RHI, TEMPI)
         ELSE IF (WI(1)+WI(4)+WI(5)+WI(6)+WI(7)+WI(8) .LE. 6.d0*TINY) THEN !Ca,K,Mg,Na,Cl,NO3=0
            CALL ISRP1F (WI, RHI, TEMPI)
         ELSE IF (WI(1)+WI(5)+WI(6)+WI(7)+WI(8) .LE. 5.d0*TINY) THEN       !Ca,K,Mg,Na,Cl=0
            CALL ISRP2F (WI, RHI, TEMPI)
         ELSE IF (WI(6)+WI(7)+WI(8) .LE. 3.d0*TINY) THEN                   !Ca,K,Mg=0
...
C=======================================================================
C
C     SUBROUTINE SETPARM (WFTYPI,  IACALCI, EPSI, MAXITI, NSWEEPI,
C    &                    EPSACTI, NDIVI, NADJI)
C     use isrpia
...
C
C     IF (WFTYPI .GE. 0)   WFTYP  = WFTYPI
C     IF (IACALCI.GE. 0)   IACALC = IACALCI
C     IF (EPSI   .GE.ZERO) EPS    = EPSI
C     IF (MAXITI .GT. 0)   MAXIT  = MAXITI
C     IF (NSWEEPI.GT. 0)   NSWEEP = NSWEEPI
C     IF (EPSACTI.GE.ZERO) EPSACT = EPSACTI
C     IF (NDIVI  .GT. 0)   NDIV   = NDIVI
C     IF (NADJI  .GE. 0)   NADJ   = NADJI
C
...
c     XK7  = 1.817     ! (NH4)2SO4(s)     <==> 2*NH4(aq) + SO4(aq)
c     XK12 = 1.382e2   ! NH4HSO4(s)       <==> NH4(aq)   + HSO4(aq)
c     XK13 = 29.268    ! (NH4)3H(SO4)2(s) <==> 3*NH4(aq) + HSO4(aq) + SO4(aq)
...
c     XK7 = XK7 *EXP( -2.65*(T0T-1.0) + 38.570*COEF)
c     XK12= XK12*EXP( -2.87*(T0T-1.0) + 15.830*COEF)
c     XK13= XK13*EXP( -5.19*(T0T-1.0) + 54.400*COEF)
...
C     XK13= XK13*EXP( -5.19*(T0T-1.0) + 54.400*COEF)
      ...
      IF (IPROB == 0) THEN
         XK10= 5.746e-17*EXP(-74.38*(T0T-1.0) +  6.120*COEF) ! ISORR
C        XK10= 2.985e-17*EXP(-75.11*(T0T-1.0) + 13.460*COEF) ! SEQUIL
      ENDIF
...
C
      IF (IPROB > 0) THEN
         TCF      = 1.0/TEMP - T0I
         DRNH4NO3 = 0.6183 * EXP(852.*TCF)
         DRNH42S4 = 0.7997 * EXP( 80.*TCF)

         ! ADJUST FOR DRH CROSSOVER AT T<271K
...
CCC      XK4  = 3.638e6   ! HNO3(g)          <==> H(aq)     + NO3(aq) ! SEQUIL
      XK41 = 2.100e5   ! HNO3(g)          <==> HNO3(aq)
c     XK5  = 0.4799  ! NA2SO4(s)        <==> 2*NA(aq)  + SO4(aq)
C     XK6  = 1.086D-16 ! NH4CL(s)         <==> NH3(g)    + HCL(g)
c     XK7  = 1.817   ! (NH4)2SO4(s)     <==> 2*NH4(aq) + SO4(aq)
c     XK8  = 37.661  ! NACL(s)          <==> NA(aq)    + CL(aq)
c      XK10 = 5.746D-17 ! NH4NO3(s)        <==> NH3(g)    + HNO3(g) ! ISORR
CCC      XK10 = 2.985e-17 ! NH4NO3(s)        <==> NH3(g)    + HNO3(g) ! SEQUIL
C      XK11 = 2.413D4   ! NAHSO4(s)        <==> NA(aq)    + HSO4(aq)
c      XK12 = 1.382D2   ! NH4HSO4(s)       <==> NH4(aq)   + HSO4(aq)
c      XK13 = 29.268  ! (NH4)3H(SO4)2(s) <==> 3*NH4(aq) + HSO4(aq) + SO4(aq)
C      XK14 = 22.05   ! NH4CL(s)         <==> NH4(aq)   + CL(aq)
      XKW  = 1.010D-14 ! H2O              <==> H(aq)     + OH(aq)
c     XK9  = 11.977  ! NANO3(s)         <==> NA(aq)    + NO3(aq)
C
      T0T = T0/TEMP
      COEF= 1.0+LOG(T0T)-T0T
...
CCC   XK4 = XK4 *EXP( 29.47*(T0T-1.0) + 16.840*COEF) ! SEQUIL
      XK41= XK41*EXP( 29.17*(T0T-1.0) + 16.830*COEF)
c     XK5 = XK5 *EXP(  0.98*(T0T-1.0) + 39.500*COEF)
C     XK6 = XK6 *EXP(-71.00*(T0T-1.0) +  2.400*COEF)
c     XK7 = XK7 *EXP( -2.65*(T0T-1.0) + 38.570*COEF)
c     XK8 = XK8 *EXP( -1.56*(T0T-1.0) + 16.900*COEF)
c     XK9 = XK9 *EXP( -8.22*(T0T-1.0) + 16.010*COEF)
c     XK10= XK10*EXP(-74.38*(T0T-1.0) +  6.120*COEF) ! ISORR
CCC   XK10= XK10*EXP(-75.11*(T0T-1.0) + 13.460*COEF) ! SEQUIL
C     XK11= XK11*EXP(  0.79*(T0T-1.0) + 14.746*COEF)
c     XK12= XK12*EXP( -2.87*(T0T-1.0) + 15.830*COEF)
C     XK13= XK13*EXP( -5.19*(T0T-1.0) + 54.400*COEF)
C     XK14= XK14*EXP( 24.55*(T0T-1.0) + 16.900*COEF)
      XKW = XKW *EXP(-22.52*(T0T-1.0) + 26.920*COEF)
...
C
      IF (IPROB > 0) THEN
         TCF      = 1.0/TEMP - T0I
         DRNH4NO3 = 0.6183 * EXP(852.*TCF)
         DRNACL   = 0.7528 * EXP( 25.*TCF)

         !ADJUST FOR DRH "CROSSOVER" AT LOW TEMPERATURES
         DRNH4NO3 = MIN (DRNH4NO3, DRNACL)
      ENDIF
C
C *** LIQUID PHASE ******************************************************
C
...
c     XK8  = 37.661  ! NACL(s)          <==> NA(aq)    + CL(aq)
C      XK10 = 5.746D-17 ! NH4NO3(s)        <==> NH3(g)    + HNO3(g)! ISORR
c      XK10 = 4.199D-17 ! NH4NO3(s)        <==> NH3(g)    + HNO3(g)! (Mozurkewich, 1993)
...
c     XK9  = 11.977  ! NANO3(s)         <==> NA(aq)    + NO3(aq)
CCC
c     XK15 = 6.067D5   ! CA(NO3)2(s)      <==> CA(aq)    + 2NO3(aq)
c     XK16 = 7.974D11  ! CACL2(s)         <==> CA(aq)    + 2CL(aq)
c     XK17 = 1.569D-2  ! K2SO4(s)         <==> 2K(aq)    + SO4(aq)
c     XK18 = 24.016    ! KHSO4(s)         <==> K(aq)     + HSO4(aq)
c     XK19 = 0.872     ! KNO3(s)          <==> K(aq)     + NO3(aq)
c     XK20 = 8.680     ! KCL(s)           <==> K(aq)     + CL(aq)
c     XK23 = 1.079D5   ! MGS04(s)         <==> MG(aq)    + SO4(aq)
c     XK24 = 2.507D15  ! MG(NO3)2(s)      <==> MG(aq)    + 2NO3(aq)
c     XK25 = 9.557D21  ! MGCL2(s)         <==> MG(aq)    + 2CL(aq)
...
c     XK17= XK17 *EXP(-9.585*(T0T-1.0) + 45.81*COEF)
c     XK18= XK18 *EXP(-8.423*(T0T-1.0) + 17.96*COEF)
c     XK19= XK19 *EXP(-14.08*(T0T-1.0) + 19.39*COEF)
c     XK20= XK20 *EXP(-6.902*(T0T-1.0) + 19.95*COEF)
...
C
 40   EXS4 = MOLAL(5) + MOLAL(6) + 2.d0*CLC + CNH42S4 + CNH4HS4 +
     &       CNA2SO4  + CNAHSO4 - WI(2)
      ...
      IF (EXS4.LT.TINY) RETURN     ! No excess SO4, return
C
...
C
      ...
      IF (WATER > TINY4) THEN
         ...
         DIAK = SQRT( (MOLAL(1)+ALFA)**2 + 4.0*ALFA*X)
         DELT = 0.5 * (DIAK - MOLAL(1) - ALFA)
      ENDIF
C
C *** CALCULATE HCL SPECIATION IN THE GAS PHASE *************************
C
      GHCL     = MAX(X-DELT, ZERO4)  ! GAS HCL
C
...
C
      IF (WATER < TINY4) RETURN
C
C *** CALCULATE HCL SPECIATION IN THE GAS PHASE *************************
C
      CALL CALCCLAQ (MOLAL(4), MOLAL(1), DELT)

      ALFAM    = GAMA(11)**2 / (XK3*R*TEMP*WATER**2)

      GASAQ(3) = DELT
      MOLAL(1) = MOLAL(1) - DELT
      MOLAL(4) = MOLAL(4) - DELT
      GHCL     = MOLAL(1) * MOLAL(4) * ALFAM
C
...
C
      X    = W(4)
      DELT = 0.0
      IF (WATER.GT.TINY4) THEN
         ALFA = XK4*R*TEMP*(WATER/GAMA(10))**2
         DIAK = SQRT( (MOLAL(1)+ALFA)**2 + 4.D0*ALFA*X)
         DELT = 0.5 * (DIAK - MOLAL(1) - ALFA)
      ENDIF
C
C *** CALCULATE HNO3 SPECIATION IN THE GAS PHASE ************************
C
      GHNO3    = MAX(X-DELT, 0.0)  ! GAS HNO3
C
...
C
      ...
      ALFAM    = GAMA(10)**2 / (XK4*R*TEMP*WATER**2)
      GASAQ(3) = DELT
      MOLAL(1) = MOLAL(1) - DELT
      MOLAL(7) = MOLAL(7) - DELT
      GHNO3    = MOLAL(1)*MOLAL(7)*ALFAM
...
C
      A1M  = XKW * GAMA(5)**2 / (XK2 * R * TEMP * GAMA(10)**2)

      BB   = MOLAL(1) + A1M
      DIAK = SQRT(BB*BB + 4.0 * MOLAL(3) * A1M)  ! Always > 0
      PSI  = 0.5*(DIAK - BB)                     ! One positive root
      PSI  = MAX(ZERO4, MIN(PSI,MOLAL(3)))       ! Constrict in acceptible range
C
...
C
      MOLAL(3) = MOLAL(3) - PSI          ! NH4+
      MOLAL(1) = MOLAL(1) + PSI          ! H+
C
...
C
      A1   = XK2*R*TEMP*GAMA(10)**2 / (XKW*GAMA(5)**2)
...
C
      ...
      IMPLICIT NONE
      REAL(8) :: M1, M2, M3, A3, A4, DELCL, DELNO, OMEGA
      REAL(8) :: C1, C2, C3, C3M, CHI3, CHI4
      INTEGER :: ISLV
C
...
C *** CALCULATE EQUILIBRIUM CONSTANTS ***********************************
C
      A3 = XK4*R*TEMP*WATER**2/GAMA(10)**2   ! HNO3
      A4 = XK3*R*TEMP*WATER**2/GAMA(11)**2   ! HCL
C
...
C
55    GHCL     = MAX(W(5) - MOLAL(4), ZERO)
      GHNO3    = MAX(W(4) - MOLAL(7), ZERO)
...
C *** IS THERE A LIQUID PHASE? ******************************************
C
      IF (WATER.LE.TINY4) RETURN
C
C *** CALCULATE EQUILIBRIUM CONSTANTS ***********************************
C
      A3M      = GAMA(11)**2 / (XK3*R*TEMP*WATER**2)
      A4M      = GAMA(10)**2 / (XK4*R*TEMP*WATER**2)
...
C
      CALL CALCNIAQ ( REAL(WAER(4)), MOLAL(1)+MOLAL(7)+MOLAL(4), DELT)
      MOLAL(1) = MOLAL(1) - DELT
      MOLAL(7) = WAER(4)  - DELT  ! NO3- = Waer(4) minus any turned into (HNO3aq)
      GASAQ(3) = DELT
C
      CALL CALCCLAQ ( REAL(WAER(5)), MOLAL(1)+MOLAL(7)+MOLAL(4), DELT)
      MOLAL(1) = MOLAL(1) - DELT
      MOLAL(4) = WAER(5)  - DELT  ! Cl- = Waer(4) minus any turned into (HNO3aq)
      GASAQ(2) = DELT
C
      GHNO3    = MOLAL(1) * MOLAL(7) * A4M
      GHCL     = MOLAL(1) * MOLAL(4) * A3M
C
...
C
      A22W = XK22 * RH * WATER * (GAMA(8)/GAMA(9))**2
C
C *** FIND ROOT
C
      BB   = NH4I + OHI + A22W
      DD   = SQRT(BB * BB - 4. * NH4I * OHI)
C
...
C
      ...
      IF (DELT < 0.0) THEN
         ...
         DELT = 0.5 * (BB + DD)
         IF (DELT > NH4I .OR. DELT > OHI) DELT = 0.0
...
C
      ...
      REAL, INTENT(IN   ) :: GGNH3
      REAL, INTENT(INOUT) :: NH4I, OHI
      REAL, INTENT(  OUT) :: NH3AQ
      REAL                :: A22W, ALF1, BB, DEL
C
C *** EQUILIBRIUM CONSTANTS
C
      A22W = XK22*RH*WATER*(GAMA(8)/GAMA(9))**2 ! GAMA(NH3) ASSUMED 1
C
C *** FIND ROOT
C
      ALF1 = NH4I - GGNH3
      BB   = ALF1 + A22W
      DEL  = 0.5 * (SQRT(BB*BB+4.0*A22W*GGNH3) - BB)
C
...
C
      ...
      OHI   = DEL
      IF (DEL <= TINY) THEN
         OHI = SQRT(XKW*RH)*WATER        ! If solution is neutral.
      ELSE
         OHI = DEL
      ENDIF
      NH3AQ = GGNH3 - DEL
...
C
      ...
      IF ( DELT < 0.0 ) THEN
         DELT = 0.5 * (BB + DD)
         IF (DELT < 0.0 .OR. DELT > CLI .OR. DELT > HI) DELT = 0.0
      ENDIF
...
C
      ...
      REAL, INTENT(INOUT) :: GGCL, CLI, HI
      REAL, INTENT(  OUT) :: CLAQ
      REAL                :: A32, ALF1, COEF, DEL1
C
C *** EQUILIBRIUM CONSTANTS
C
      A32  = XK32*WATER/GAMA(11)**2 ! GAMA(HCL) ASSUMED 1
...
C
      ALF1  = CLI - GGCL
      COEF  = ALF1 + A32
      DEL1  = 0.5*(SQRT(COEF*COEF+4.*A32*GGCL) - COEF)
C
C *** CORRECT CONCENTRATIONS
C
      CLI  = ALF1 + DEL1
      IF (DEL1 <= TINY) then
         HI = SQRT(XKW*RH)*WATER         ! If solution is neutral.
      ELSE
         HI = DEL1
      ENDIF
      CLAQ = GGCL - DEL1
...
C
      BB = NO3I + HI + A42
      DD = SQRT(BB * BB - 4.0 * NO3I * HI)

C
C *** GET APPROPRIATE ROOT.
C
      DELT = 0.5 * (BB - DD)
      IF (DELT < 0.0 .OR. DELT > HI .OR. DELT > NO3I) THEN
         DELT = 0.5 * (BB + DD)
         IF (DELT < 0.0 .OR. DELT > HI .OR. DELT > NO3I) DELT = 0.0
      ENDIF
C
...
C
      ...
      REAL, INTENT(IN   ) :: GGNO3
      REAL, INTENT(INOUT) :: NO3I, HI
      REAL, INTENT(  OUT) :: NO3AQ

      REAL :: A42, ALF1, BB, CC, DEL1
C
C *** EQUILIBRIUM CONSTANTS
C
      A42  = XK42*WATER/GAMA(10)**2 ! GAMA(HNO3) ASSUMED 1
...
C
      ALF1  = NO3I - GGNO3
      BB    = HI + ALF1 + A42
      CC    = A42*GGNO3 - HI*ALF1
      DEL1  = 0.5*(SQRT(BB*BB+4.*CC) - BB)
C
C *** CORRECT CONCENTRATIONS
C
      NO3I  = ALF1 + DEL1
      HI    = HI   + DEL1
      IF (HI <= TINY) HI = SQRT(XKW*RH)*WATER   ! If solution is neutral.
      NO3AQ = GGNO3 - DEL1
...
C
      ...
      REAL, INTENT(IN ) :: HI, SO4I, HSO4I
      REAL, INTENT(OUT) :: DELTA

      REAL :: BB, CC, DD, SQDD, A8
C
C *** IF TOO LITTLE WATER, DONT SOLVE
C
      IF (WATER .LE. 10.*TINY4) THEN
         DELTA = 0.0
         RETURN
      ENDIF
C
...
C
      A8 = XK1 * WATER * GAMA(8)**2 / GAMA(7)**3
C
      BB = HI + SO4I + A8
      CC = HI*SO4I - HSO4I*A8
      DD = BB*BB - 4.0*CC
...
C
      ...
      REAL, INTENT(IN ) :: GG
      REAL, INTENT(OUT) :: HI, OHI

      REAL :: CN, DD

      CN = SQRT(XKW*RH) * WATER
      DD = SQRT(GG * GG + 4. * CN * CN)
      DD = MAX(DD, 2.*CN, ABS(GG))
C
...
C
      ...
      IF (W(1)+W(4)+W(5)+W(6)+W(7)+W(8) .LE. 6.d0*TINY) THEN  !Ca,K,Mg,Na,Cl,NO3=0
         CALL CALCACT1
      ELSEIF (W(1)+W(5)+W(6)+W(7)+W(8) .LE. 5.d0*TINY) THEN   !Ca,K,Mg,Na,Cl=0
         CALL CALCACT2
      ELSEIF (W(6)+W(7)+W(8) .LE. 3.d0*TINY) THEN             !Ca,K,Mg=0
          CALL CALCACT3
       ELSE
          CALL CALCACT4
       ENDIF
...
C
      ...
      REAL    :: AGAMA,SION,H,CH,F1(3),F2(4)
      REAL    :: XIJ, YJI, GZH, GAMIN8, ION, IONIC2
      INTEGER :: I, J

      REAL, PARAMETER :: AGFC = 0.511 * 298.0 ** 1.5
C
...
C
      ION = 0.0
      DO I=1,NIONS
         ION = ION + MOLAL(I)*ZSQ(I)
      ENDDO

      IONIC2 = MAX(MIN(ION/WATER,100.0), TINY4)
C
C *** CALCULATE BINARY ACTIVITY COEFFICIENTS ***************************
C
      IF (IONIC < TINY4 .OR. ABS(IONIC - IONIC2) > 0.01) THEN
         CALL KMTAB(IONIC2, TEMP, G0)
      ENDIF
      IONIC = IONIC2
C
C *** CALCULATE MULTICOMPONENT ACTIVITY COEFFICIENTS *******************
C
      AGAMA = agfc / temp**1.5    ! Debye Huckel const. at T
      SION  = SQRT(IONIC)
      H     = AGAMA*SION/(1.+SION)
      ...
      DO I=1,3
         DO J=1,4
            GZH   = G0(I,J) + ZIJ(I,J+3) * H
            CH    = ZIJS(I,J+3) * GZH / ION
            XIJ   = CH*MOLAL(I)
            YJI   = CH*MOLAL(J+3)
            F1(I) = F1(I) + YJI
            F2(J) = F2(J) + XIJ
...
!     GAMA(13) = 0.6*GAMA(04)+0.4*GAMA(09))         ! LC ; SCAPE
!     GAMA(14) = 0.1d0                              ! CASO4
...
C
      DO I = 5, 11 !NPAIR
         GAMA(I) = EXP( ZZL10(I) * GAMA(I) ) !agama )
      ENDDO
C
C *** SETUP ACTIVITY CALCULATION FLAGS ********************************
C
      CALAIN = ABS(GAMIN8-GAMA(8)) > EPSACT * MIN(GAMIN8, GAMA(8))
      IF (FRST) THEN
         CALAOU = CALAIN
         FRST   = .FALSE.
      ENDIF
      ...
      REAL FUNCTION GA(I,J)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: I, J

      GA = (F1(I)*ZM(I) + F2(J)*ZM(J+3)) * ZIJM(I,J+3) - H
...
C
      ...
      G = (F1(I)*ZM(I) + F2(J)*ZM(J+3)) * ZIJM(I,J+3) - H
...
C=======================================================================
      ...
      USE ISRPIA, ONLY: GAMA, CALAOU, FRST
      IMPLICIT NONE

      REAL, PARAMETER :: GTHRESH = 100.0

      IF (ANY(GAMA > GTHRESH)) THEN
         GAMA   = 0.1
         CALAOU = .TRUE.
         FRST   = .TRUE.
      ENDIF
...
C=======================================================================
C
      SUBROUTINE KMTAB (IN,TEMP,G)

      IMPLICIT NONE

      REAL, INTENT( IN) :: IN, TEMP
      REAL, INTENT(OUT) :: G(3,4)

      IF (TEMP < 210.0) THEN

         CALL KM198 (IN,G)

      ELSEIF (TEMP < 235.0) THEN

         CALL KM223 (IN,G)

      ELSEIF (TEMP < 260.0) THEN

         CALL KM248 (IN,G)

      ELSEIF (TEMP < 285.0) THEN

         CALL KM273 (IN,G)

      ELSEIF (TEMP < 310.0) THEN
...
C=======================================================================
C
      SUBROUTINE KM198 (IONIC,GXX)

      implicit none

      real, intent( in) :: IONIC
      real, intent(out) :: GXX(3,4)
      integer           :: ipos
      real              :: bncm(3,4,561)
C
C *** NaCl
...
C *** (NH4)3H(SO4)2
C
!      data bncm(4,2,:) / 561 * 0.0 /
C
C *** CASO4
C
!     data bncm(4,3,:) / 561 * 0.0 /
C
C *** CANO32
C
!     data bncm(4,4,:) /
...
C=======================================================================
      ...
      REAL(8)              :: X(3), Q, R, D, SQD, R2, Q3, SMALL
      REAL(8)              :: THET, COEF, SSIG, TSIG, S, T
      INTEGER              :: I, IX
      ...
      REAL(8), PARAMETER   :: EPS   = 1.0D-50
      REAL(8), PARAMETER   :: EPS8  = 8.D0*EPSILON(1.D0)
C
...
C
         ...
         SMALL = EPS8 * MAX(R2, ABS(Q3))

C *** CALCULATE ROOTS *************************************************
C
C  D < 0, THREE REAL ROOTS
C
         IF (D < -SMALL) THEN        ! D < -EPS  : D < ZERO
            IX   = 3
            THET = EXPON*ACOS(R/SQRT(-Q3))
            COEF = 2.D0*SQRT(-Q)
...
C
         ELSE IF (D <= SMALL) THEN    ! -EPS <= D <= EPS  : D = ZERO
            IX   = 2
            SSIG = SIGN (1.D0, R)
            S    = SSIG*(ABS(R))**EXPON
            X(1) = 2.D0*S  - EXPON*A1
