...
contains
  ...
  subroutine load_cgrid ( spc_cat )
    ...
    INTEGER :: LSULF, LO3, LCH4                  ! Gas chem CGRID index
    ...
    IF ( SPC_CAT .EQ. 'GC' ) THEN
       ...
       LCH4 = INDEX1( 'ECH4', N_GC_SPC, GC_SPC )
    ...
    ELSE IF ( SPC_CAT .EQ. 'TR' ) THEN
       ...
       WRITE( LOGDEV, fmt9 ) 'Transported inert tracer gas species not used'
    ...
    DO SPC = 1, N_SPCS
       ...
       IF ( NDX .GT. 0 ) THEN
          ...
          !$omp parallel private(press_in,vec,press_out)
          !$omp do private(j,iw,ka,k,psfc)
          do j = 1, jtab_w(jtw_init)%jend(1)
             ...
             if (V == LO3 .and. SPC_CAT == 'GC') then

                do k = ka, mza-1
                   if (press(k,iw) < 200.d2) exit
                enddo
                cgrid(ka:k,iw,v) = max( ICBC_FAC( SPC ) * vec(1:k-ka+1), cmin)

             else

                cgrid(ka:mza,iw,v) = max( ICBC_FAC( SPC ) * vec(1:mza-ka+1), cmin)

             endif

             cgrid(1:ka-1,iw,v) = cgrid(ka,iw,v)

             if (V == LCH4 .and. SPC_CAT == 'GC') then

!               cgrid(1:mza,iw,v) = cgrid(1:mza,iw,v) + ATM_CH4
                cgrid(1:mza,iw,v) = ATM_CH4

             endif

             ! Reduce aeresol mass above Ptop using the local density
             IF ( SPC_CAT .EQ. 'AE' ) THEN

                do k = ka, mza-1
                   if (press(k,iw) < ptop) exit
                enddo

                if (k < mza) then
                   cgrid(k:mza,iw,v) = cgrid(k:mza,iw,v) * real(rho(k:mza,iw)) / real(rho(k-1,iw))
                endif
