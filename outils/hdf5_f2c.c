...
/* Fortran to C - dimensions should be reversed */
/* for (i = 0; i < ndims; i++) {  dims[i] = dimsc[i];   } */
for (i = 0; i < ndims; i++) {  dims[i] = (int)dimsc[ndims-i-1];   }
/*printf("fh5d_get_dims: %d %d %d %d\n",ndims,dimsc[0],dimsc[1],dimsc[2]);
 */
return;
}
...
for (i = 0; i < *ndims; i++)
  {
    ...
    j = *ndims-i-1;
    dimsc[i] = (hsize_t)dims[j]; 
    maxdims[i] = (hsize_t)dims[j]; 
    count[i] = (hsize_t)dims[j]; 
  }
