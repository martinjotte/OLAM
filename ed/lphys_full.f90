...
subroutine lphysiol_full(T_L,  &
  ...
  use lphys_interface, only: prep_lphys_solution, exact_lphys_solution, &
                             store_exact_lphys_solution, fill_lphys_sol_exact, &
                             fill_lphys_sol_exact, fill_lphys_sol_approx
...
subroutine c3solver(met,apar,gsdata,sol,ilimit)
  use c34constants
  use lphys_interface, only: solve_open_case_c3, solve_closed_case_c3, &
                             setapar_c3, closed2open
  implicit none
...
subroutine c4solver(met,apar,gsdata,sol,ilimit)
  use c34constants
  use lphys_interface, only: solve_closed_case_c4, solve_open_case_c4, &
                             setapar_c4, closed2open
  implicit none
...
real function residual_c3(gsdata,met,apar,x)
  use c34constants
  use lphys_interface, only: aflux_c3, csc_c3, quad4ci
  implicit none
...
real function residual_c4(gsdata,met,apar,x)
  use c34constants
  use lphys_interface, only: aflux_c4, csc_c4
  implicit none
...
subroutine zbrak_c3(gsdata,met,apar,x1,x2,n,xb1,xb2,nb)
  use c34constants
  use lphys_interface, only: residual_c3
  implicit none
...
real function zbrent_c3(gsdata,met,apar,x1,x2,tol,success_flag)
  ...
  do iter = 1,itmax
     ...
     fb = residual_c3(gsdata,met,apar,b)
  enddo
  write(*,*) 'ZBRENT_C3 exceeding maximum iterations.'
  zbrent_c3=b
  return
...
subroutine zbrak_c4(gsdata,met,apar,x1,x2,n,xb1,xb2,nb)
  use c34constants
  use lphys_interface, only: residual_c4
  implicit none
...
real function zbrent_c4(gsdata,met,apar,x1,x2,tol,success_flag)
  ...
  do iter = 1,itmax
     ...
     fb = residual_c4(gsdata,met,apar,b)
  enddo
  write(*,*) 'ZBRENT_C4 exceeding maximum iterations.'
  zbrent_c4=b
  return
...
subroutine solve_open_case_c3(gsdata,met,apar,sol,ilimit,success_flag)
  use c34constants
  use lphys_interface, only: aflux_c3, csc_c3, zbrent_c3, quad4ci, &
                             zbrak_c3, closed2open
  implicit none
...
subroutine solve_open_case_c4(gsdata,met,apar,sol,ilimit,success_flag)
  use c34constants
  use lphys_interface, only: zbrent_c4, zbrak_c4, closed2open
  implicit none
...
real function quad4ci(gsdata,met,apar,x,success)
  ...
  sol_flag(1:2) = 1
  do isol = 1,2
!     if(ci(isol) <= 0.0 .or. isnan(ci(isol)))sol_flag(isol) = 0
! fix for machines with no 'isnan'
...
real function arrhenius(T,c1,c2)
  implicit none
  real :: T,c1,c2
  arrhenius = c1*exp(c2*(1.0/288.15-1.0/(T+273.15)))
  return
...
subroutine exact_lphys_solution(photosyn_pathway, met, apar, gsdata, sol,   &
  ...
  use c34constants
  use lphys_interface, only: c3solver, c4solver
...
subroutine store_exact_lphys_solution(old_st_data, met, prss,   &
  ...
  use c34constants
  use lphys_interface, only: residual_c3, residual_c4, setapar_c3, setapar_c4
...
subroutine fill_lphys_sol_approx(gsdata, met, apar, old_st_data, sol,   &
  ...
  use c34constants
  use lphys_interface, only: aflux_c3, aflux_c4, quad4ci, &
                             setapar_c3, setapar_c4, &
                             solve_closed_case_c3, solve_closed_case_c4
