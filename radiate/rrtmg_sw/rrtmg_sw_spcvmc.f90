...
! collimated beam
         ...
         zdenr = zrp1 * zrm1 * ( zrk * zemp1 + zgamma1 * zemm1 )

         zdenr = zw * zdenr / max( zdenr * zdenr, 1.e-9 )

         pref(ig,jk) = (zr1 - zr2*zemm - zr3*zem2*zem1) * zdenr

         ptra(ig,jk) = zem2 - (zem2*zt1 - zem2*zt2*zemm - zt3*zem1) * zdenr

         pref(ig,jk) = max(pref(ig,jk), 0.0 )
         ptra(ig,jk) = min(ptra(ig,jk), 1.0 )
         ptra(ig,jk) = max(ptra(ig,jk), zem2)
