...
!     call swdatinit(cpdair)
      ...
      if (.not. exists) then
         write(io6,*) "rrtmg_ls_init: Error opening data file " // trim(inputfile)
         write(io6,*) "RRTMg shortwave datafile cannot be found."
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg16, only : kao, kbo, selfrefo, forrefo, sfluxrefo, &
                            absa, absb, selfref, forref, sfluxref, rayl
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg17, only : kao, kbo, selfrefo, forrefo, sfluxrefo, &
                            absa, absb, selfref, forref, sfluxref, &
                            dsfluxref, rayl
...
! ------- Local -------
      ...
      do jp = 1,4
         do igc = 1,ngc(2)
            dsfluxref(igc,jp) = sfluxref(igc,jp+1) - sfluxref(igc,jp)
         enddo
      enddo
      dsfluxref(1:ngc(2),5) = dsfluxref(1:ngc(2),4)

      do igc = 1, ngc(2)
         raylt(ngs(1)+igc) = rayl
      enddo
...
! ------- Local -------
      ...
      do jp = 1,8
         ...
         enddo
      enddo
      dsfluxref(1:ngc(3),9) = dsfluxref(1:ngc(3),8)

      do igc = 1, ngc(3)
         raylt(ngs(2)+igc) = rayl
      enddo
...
! ------- Local -------
      ...
      do jp = 1,8
         ...
         enddo
      enddo
      dsfluxref(1:ngc(4),9) = dsfluxref(1:ngc(4),8)

      do igc = 1, ngc(4)
         raylt(ngs(3)+igc) = rayl
      enddo
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg20, only : kao, kbo, selfrefo, forrefo, sfluxrefo, absch4o, &
                            absa, absb, selfref, forref, sfluxref, absch4, rayl
...
! ------- Local -------
      ...
      do jp = 1,8
         ...
         enddo
      enddo
      dsfluxref(1:ngc(6),9) = dsfluxref(1:ngc(6),8)

      do igc = 1, ngc(6)
         raylt(ngs(5)+igc) = rayl
      enddo
...
! ------- Local -------
      ...
      do jp = 1,8
         ...
         enddo
      enddo
      dsfluxref(1:ngc(7),9) = dsfluxref(1:ngc(7),8)

      do igc = 1, ngc(7)
         raylt(ngs(6)+igc) = rayl
      enddo
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg23, only : kao, selfrefo, forrefo, sfluxrefo, raylo, &
                            absa, selfref, forref, sfluxref, rayl
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg24, only : kao, kbo, selfrefo, forrefo, sfluxrefo, &
                            abso3ao, abso3bo, raylao, raylbo, absa, &
                            absb, selfref, forref, sfluxref, drayla, &
                            abso3a, abso3b, rayla, raylb, dsfluxref
...
! ------- Local -------
      ...
      do jp = 1,8
         do igc = 1,ngc(9)
            dsfluxref(igc,jp) = sfluxref(igc,jp+1) - sfluxref(igc,jp)
            drayla   (igc,jp) = rayla   (igc,jp+1) - rayla   (igc,jp)
         enddo
      enddo
      dsfluxref(1:ngc(9),9) = dsfluxref(1:ngc(9),8)
      drayla   (1:ngc(9),9) = drayla   (1:ngc(9),8)

      do igc = 1, ngc(9)
         raylt(ngs(8)+igc) = 0.5 * (rayla(igc,1) + rayla(igc,2))
      enddo
...
!-----------------------------------------------------------------------
      ...
      use rrsw_kg28, only : kao, kbo, sfluxrefo, absa, absb, &
                            sfluxref, dsfluxref, rayl
...
! ------- Local -------
      ...
      do jp = 1,4
         ...
         enddo
      enddo
      dsfluxref(1:ngc(13),5) = dsfluxref(1:ngc(13),4)

      do igc = 1, ngc(13)
         raylt(ngs(12)+igc) = rayl
      enddo
