...
! ------- Modules -------
      ...
      use rrlw_wvn, only: totplnk, dtotplnk, totplnkderiv, dtotplnkderiv
      use rrlw_ref

      implicit none

      private
      public :: setcoef, lwavplank

      contains

!----------------------------------------------------------------------------
      subroutine setcoef(nlayers, nsfc, isfc_dt, pavel, tavel, tbound, semiss, &
...
! ----- Input -----
      ...
      real(kind=rb), intent(in) :: n2ovmr(nlayers)     ! Nitrous oxide volume mixing ratio

      real(kind=rb), intent(in) :: o2vmr(nlayers)      ! Oxygen volume mixing ratio

      real(kind=rb), intent(in) :: covmr(nlayers)      ! CO volume mixing ratio

      real(kind=rb), intent(in) :: cfc11vmr(nlayers)   ! CFC11 volume mixing ratio

      real(kind=rb), intent(in) :: cfc12vmr(nlayers)   ! CFC12 volume mixing ratio

      real(kind=rb), intent(in) :: cfc22vmr(nlayers)   ! CFC22 volume mixing ratio

      real(kind=rb), intent(in) :: ccl4vmr(nlayers)    ! CCL4 volume mixing ratio

! ----- Output -----

      real(kind=rb), intent(out) :: wx(nlayers,maxxsec)! cross-section amounts (mol/cm-2)
      ...
      real(kind=rb), intent(out) :: planklay(nbndlw,nlayers)

      real(kind=rb), intent(out) :: plankbnd(nbndlw,nsfc)

      real(kind=rb), intent(out) :: dplankbnd_dt(nbndlw,nsfc)
      ...
      real(kind=rb), intent(out) :: coln2o(nlayers)  ! column amount (n2o)

      real(kind=rb), intent(out) :: colco(nlayers)   ! column amount (co)

      real(kind=rb), intent(out) :: colch4(nlayers)  ! column amount (ch4)

      real(kind=rb), intent(out) :: colo2(nlayers)   ! column amount (o2)

      real(kind=rb), intent(out) :: colbrd(nlayers)  ! column amount (broadening gases)
      ...
      integer(kind=im), intent(out) :: indminor(nlayers)

      real(kind=rb), intent(out) :: minorfrac(nlayers)

      real(kind=rb), intent(out) :: scaleminor(nlayers)

      real(kind=rb), intent(out) :: scaleminorn2(nlayers)

      real(kind=rb), intent(out), dimension(nlayers) :: &
                       fac00, fac01, &
                       fac10, fac11
...
! ----- Local -----
      ...
      integer(kind=im) :: lay, jp1, indlay, iband, indbound
      real   (kind=rb) :: tbndfrac, tlayfrac, summol, wbroad, planksfc
      real   (kind=rb) :: plog, fp, ft, ft1, scalefac, factor, compfp, cdf

      real(kind=rb), parameter :: stpfac = 296._rb/1013._rb

! Calculate the integrated Planck functions at each layer and level

      do lay = 1, nlayers
         indlay   = max(1, min(180, int(tavel(lay) - 159._rb)))
         tlayfrac = tavel(lay) - 159._rb - real(indlay)

         do iband = 1, 16
            planklay(iband,lay) = totplnk(iband,indlay) + tlayfrac * dtotplnk(iband,indlay)
         enddo
      enddo

! Planck functions for lowest level and shaved cells

      do lay = 1, nsfc
         indbound = max(1, min(180, int(tbound(lay) - 159._rb)))
         tbndfrac = tbound(lay) - 159._rb - real(indbound)

         do iband = 1, 16
            planksfc = totplnk(iband,indbound) + tbndfrac * dtotplnk(iband,indbound)
            plankbnd(iband,lay) = semiss(iband,lay) * planksfc
         enddo

         if (isfc_dt == 1) then
            do iband = 1, 16
               planksfc = totplnkderiv(iband,indbound) + tbndfrac * dtotplnkderiv(iband,indbound)
               dplankbnd_dt(iband,lay) = semiss(iband,lay) * planksfc
            enddo
         endif
      enddo
...
!  Calculate needed column amounts
      ...
      do lay = 1, nlayers
         ...
         summol = co2vmr(lay) + o3vmr (lay) + n2ovmr(lay) &
                + covmr (lay) + ch4vmr(lay) + o2vmr (lay)
         ...
         coln2o(lay) = cdf * n2ovmr(lay)
         colco (lay) = cdf * covmr (lay)
         colch4(lay) = cdf * ch4vmr(lay)

         wx(lay,1) = cdf * ccl4vmr (lay)
         wx(lay,2) = cdf * cfc11vmr(lay)
         wx(lay,3) = cdf * cfc12vmr(lay)
         wx(lay,4) = cdf * cfc22vmr(lay)

         wbroad      = 1._rb - summol
         colbrd(lay) = cdf * wbroad
...
!  two values that the layer pressure lies.
         ...
         plog = log(pavel(lay))
         jp(lay) = max(1, min(58, int(36._rb - 5.*(plog+0.04_rb))))
         jp1 = jp(lay) + 1
...
!  layer temperature falls.
         ...
         jt1(lay) = max(1, min(4, int(3._rb + (tavel(lay)-tref(jp1))/15._rb)))
         ft1 = ((tavel(lay)-tref(jp1))/15._rb) - real(jt1(lay)-3)

         ft  = min(max(ft,  -2.0), 2.0)
         ft1 = min(max(ft1, -2.0), 2.0)
...
!  the optical depths (performed in routines TAUGBn for band n).`
         ...
         scalefac          = scaleminor(lay) / (1.0+h2ovmr(lay))
         forfac      (lay) = stpfac * scalefac * colh2o(lay)
         scaleminorn2(lay) = wbroad * scalefac
      enddo
...
!  species parameter in lower atmosphere.
         rat_h2oco2  (lay)=chi_mls1_2(jp(lay))  *colco2(lay)
         rat_h2oco2_1(lay)=chi_mls1_2(jp(lay)+1)*colco2(lay)

         rat_h2oo3  (lay)=chi_mls1_3(jp(lay))  *colo3(lay)
         rat_h2oo3_1(lay)=chi_mls1_3(jp(lay)+1)*colo3(lay)

         rat_h2on2o  (lay)=chi_mls1_4(jp(lay))  *coln2o(lay)
         rat_h2on2o_1(lay)=chi_mls1_4(jp(lay)+1)*coln2o(lay)

         rat_h2och4  (lay)=chi_mls1_6(jp(lay))  *colch4(lay)
         rat_h2och4_1(lay)=chi_mls1_6(jp(lay)+1)*colch4(lay)

         rat_n2oco2  (lay)=chi_mls4_2(jp(lay))  *colco2(lay)
         rat_n2oco2_1(lay)=chi_mls4_2(jp(lay)+1)*colco2(lay)
      enddo
...
!  species parameter in upper atmosphere.
         ...
         rat_o3co2  (lay)=chi_mls3_2(jp(lay))  *colco2(lay)
         rat_o3co2_1(lay)=chi_mls3_2(jp(lay)+1)*colco2(lay)
      enddo
...
!***************************************************************************
      ...
      do in = 1, 180
         do ib = 1, 16
            dtotplnk(ib,in) = totplnk(ib,in+1) - totplnk(ib,in)
         enddo
      enddo
      dtotplnk(1:16,181) = dtotplnk(1:16,180)
...
!***************************************************************************
      ...
      if (allocated(totplnkderiv)) return

      allocate( totplnkderiv(nbndlw,181))
      allocate(dtotplnkderiv(nbndlw,181))
      ...
      do in = 1, 180
         do ib = 1, 16
            dtotplnkderiv(ib,in) = totplnkderiv(ib,in+1) - totplnkderiv(ib,in)
         enddo
      enddo
      dtotplnkderiv(1:16,181) = dtotplnkderiv(1:16,180)
