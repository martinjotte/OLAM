...
! ------- Modules -------
      ...
      use rrlw_wvn, only: totplnk, dtotplnk
      use rrlw_ref
...
!----------------------------------------------------------------------------
      subroutine setcoef(nlayers, nsfc, pavel, tavel, tbound, semiss, &
                         coldry, wx, h2ovmr, o3vmr, co2vmr, ch4vmr, covmr,&
                         n2ovmr, o2vmr, cfc11vmr, cfc12vmr, cfc22vmr, ccl4vmr, &
                         laytrop, jp, jt, jt1, planklay, dplankdn, dplankup, &
                         plankbnd, dplnksfc, colh2o, colco2, colo3, coln2o, &
...
! ----- Output -----
      ...
      real(kind=rb), intent(out) :: dplankdn(nbndlw,nlayers)
      real(kind=rb), intent(out) :: dplankup(nbndlw,nlayers)
      real(kind=rb), intent(out) :: dplnksfc(nbndlw,nsfc)
...
! Calculate the integrated Planck functions at each layer and level
      ...
      do lay = 1, nlayers-1
         do iband = 1, 16
            dplankup(iband,lay)   = 0.5 * (planklay(iband,lay+1) - planklay(iband,lay  ))
            dplankdn(iband,lay+1) = 0.5 * (planklay(iband,lay  ) - planklay(iband,lay+1))
         enddo
      enddo

      ! lowest level and shaved cells
      do lay = 1, nsfc
         ...
         do iband = 1, 16
            ...
            dplnksfc(iband,lay) = planksfc - planklay(iband,lay)
            plankbnd(iband,lay) = semiss(iband,lay) * planksfc
         enddo
      enddo

      dplankup(1:16,nlayers) = 0.0
      dplankdn(1:16,1)       = dplnksfc(1:16,1)
