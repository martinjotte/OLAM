...
!       par_diffuse_ks (krad,iw) = par_diffuse_k(krad)
  ...
  if (ilwrtyp > 0) then
     ...
     do k = ka, ka + nsfc - 1
        krad = k - koff
        flux_net_bot =        frac_sfck(krad,iw)  * (lwuflxt_sfc(krad) - lwdflxt_sfc(krad)) &
                     + (1.0 - frac_sfck(krad,iw)) * flux_net(k-1)
...
!    endif
     ...
     pbl_cld_forc(iw) = pbl_cld_forc(iw) &
                      - sum( fthrd_lw(krad1:krad2,iw) * dzt(krad1:krad2) ) / real(rho(kpblh(iw),iw))
