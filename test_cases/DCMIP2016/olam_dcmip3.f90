subroutine olam_dcmip_init()
  ...
  use mem_basic,   only: vc, vmc, wc, wmc, rho, thil, theta, tair, &
                         press, rr_w, rr_v, ue, ve, vxe, vye, vze
  use mem_ijtabs,  only: jtab_v, jtab_w, itab_v, jtv_init, jtw_init
  use mem_grid,    only: mza, mwa, zt, xev, yev, zev, vnx, vny, vnz, lpw, &
                         glatw, glonw, lpv, zm, glatv, glonv, &
                         gdz_belo8, gdz_abov8, vxn_ew, vyn_ew, &
                         vxn_ns, vyn_ns, vzn_ns
  ...
  real(r8) :: latrad,lonrad,latdeg,londeg
  real(r8) :: p,t,phis,ps,q,q1,q2,q3,q4
  real(r8) :: raxis, uv01dx, uv01dy, uv01dz, uv01dr
  real(r8) :: hyam, hybm, gc, xscal, cl, cl2
  real(r8) :: zm0, zt0, rhom0, rhot0, u0, v0, wm0, wt0, rm0, rt0, thetav
  real(r8) :: rho_tot(mza), pkhyd, ut0, vt0
  integer  :: j,iw,k,iv,iw1,iw2,zcoords,rcoords,cfv,shear,moist,iter,ka,kbc, &
...
!==============================================================================
           ...
           call test1_advection_orography(lonrad,latrad,p,zt0,zcoords, &
                cfv,hybrid_eta,hyam,hybm,gc,ut0,vt0,wt0, &
...
!==============================================================================
           ...
           call test2_steady_state_mountain(lonrad,latrad,p,zt0,zcoords, &
                hybrid_eta,hyam,hybm,ut0,vt0,wt0, &
                t,phis,ps,rhot0,q)
...
!==============================================================================
           ...
           call baroclinic_wave_test(deep,moist,pertt,Xscal,lonrad,latrad,p,zt0,zcoords, &
                                     ut0,vt0,t,thetav,phis,ps,rhot0,q)
...
!==============================================================================
           ...
           call supercell_test(lonrad,latrad,p,zt0,zcoords,ut0,vt0,t,thetav,ps,rhot0,q,pert)
...
!  Assume: q is mixing ratio, rhot0 include dry air and water vapor
        ...
        ! Initializing v[xyz]e's here is necessary for cells that
        ! intersect topography with the new prognostic shaved-cell method.

        vxe  (k,iw) = vxn_ew(iw) * ut0 + vxn_ns(iw) * vt0
        vye  (k,iw) = vyn_ew(iw) * ut0 + vyn_ns(iw) * vt0
        vze  (k,iw) =                    vzn_ns(iw) * vt0
...
! MPI parallel send/recv of W group
  ...
  call lbcopy_w(a1=wc, a2=wmc, a3=thil, d1=rho, d2=press)
...
subroutine olam_dcmip_prescribedflow(time0,rho_old)
  ...
  use mem_addsc,   only: addsc
  use consts_coms, only: p00, rocp, erad, pio180, p00i, r8
  use misc_coms,   only: dtsm, iparallel, nrk_scal
  ...
  real(r8) :: raxis, uv01dx, uv01dy, uv01dz, uv01dr
  real(r8) :: zm0, zt0, rhom0, rhot0, u0, v0, wm0, wt0, dts

  integer  :: j,iw,k,iv,iw1,iw2,zcoords,ka,kb,jv
...
! We assume that the prescribed-flow test cases do not have topography,
! or we would need to prescribe the v[xyx]e's for cells that intersect
! terrain for the prognostic shaved-cell method.

  call diagvel_t3d()

  if (iparallel == 1) call mpi_send_w(dvara1=rho)
...
use mem_basic,   only: rho, thil, press, rr_v, vxe, vye, vze
use mem_ijtabs,  only: jtab_w, jtw_prog, jtv_prog
use mem_grid,    only: mza, zt, lpw, glatw, glonw, zm, xew, yew, zew
...
real(r8) :: precl, vx, vy, vz

integer :: j,iw,k,nz,ka
...
! tendencies for DCMIP test cases.
  ...
  use mem_grid,   only: mza, lpw, glatw, glonw
  ...
  integer  :: iw, k, ka, j
  real(r8) :: dt, latdeg, londeg, cl, cl2, cl_f, cl2_f
...
!----------------------------------------------------------------------
  !$omp parallel do private(iw,latdeg,londeg,ka,k,cl,cl2,cl_f,cl2_f)
  do j = 1,jtab_w(jtw_prog)%jend; iw = jtab_w(jtw_prog)%iw(j)
!----------------------------------------------------------------------
     ...
     do k = ka,mza
      ...
      ! Update chemistry tendencies only at specified time interval
        ...
        if (mod(time8p,dt) < dtlong) then
           cl  = real(addsc(1)%sclp(k,iw),r8)
           cl2 = real(addsc(2)%sclp(k,iw),r8)
