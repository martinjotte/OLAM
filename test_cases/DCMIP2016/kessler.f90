...
SUBROUTINE KESSLER(theta, qv, qc, qr, rho, pk, dt, z, nz, precl, iw)

  IMPLICIT NONE

  ! Modified (slightly) for OLAM vertical grid lowest level at ka
  !------------------------------------------------
  ...
  INTEGER :: k, ka, rainsplit, nt

  !------------------------------------------------
  !   Begin calculation
  !------------------------------------------------
  ka = 2 ! lowest prognosed level in OLAM
  f2x = 17.27d0
  ...
  do nt=1,rainsplit
    ...
    precl = precl + rho(ka) * qr(ka) * velqr(ka) / rhoqr
