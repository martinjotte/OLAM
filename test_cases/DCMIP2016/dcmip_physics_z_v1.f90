...
!     precl  (INOUT) large-scale precip rate (m/s)
...
SUBROUTINE DCMIP2016_PHYSICS(test, u, v, p, qv, qc, qr, &
                             rho, theta, exner, z, zi,  &
                             dt, lat, nz, ka, iw, precl, pbl_type, prec_type)

  use kesslerm, only: kessler
  ...
  INTEGER, INTENT(IN) :: &
            ...
            ka        ,  & ! Lowest prognosed model level
            iw             ! OLAM grid cell horizontal index
  ...
  REAL(8), INTENT(INOUT) :: &
            precl        ! Large-scale precip rate beneath the grid column (m/s)
  ...
  elseif (prec_type .eq. 0) then
    ...
    ! These are computed in case they are needed for PBL
    do k = ka,nz
      qsv(k) = qv(k) / (one + qv(k))
  ...
  !----------------------------------------------------
  ! Do not apply surface fluxes if pbl_type is 2
  !----------------------------------------------------
  if (pbl_type .eq. 2) then
    return
  endif
  ...
  v(ka) = v(ka) / (one + dt * Cd * wind / za)
  qsv(ka) = (qsv(ka) + C * wind * qsats * dt / za) / (one + C * wind * dt / za)
  t(ka) = (t(ka) + C * wind * Tsurf * dt / za) / (one + C * wind * dt / za)

  qv(ka) = qsv(ka) / (1.0 - qsv(ka))
  rhom(ka) = rho(ka) / (1.0 - qsv(ka))
