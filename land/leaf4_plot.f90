...
real, optional, intent(in) :: surface_srrv ! surface saturation mix ratio [kg_vap/kg_dryair]
real, optional, intent(in) :: ground_rrv   ! soil vapor mix ratio [kg_vap/kg_air]
...
if (present(can_depth) .and.  &
   ...
   water = water + can_depth * rhos * canrrv
...
do k = 1,nzg
 ...
 !  endif
   ...
   if (present(psi_matric)) then

      write (number,'(f12.3)') psi_matric(k)
      call o_plchhq(xw7,ywk2,trim(adjustl(number)),psiz,0.,0.)
      if (k == 1) call o_plchhq(xw7,yk2,'psi_matric ',psiz,0.,0.)
...
if (present(surface_srrv)) then

   write (number,'(f12.3)') surface_srrv * 1000.
   call o_plchhq(xw11,yw1,'surface_srrv = '//trim(adjustl(number)),psiz,0.,0.)
...
if (nlev_sfcwater == 0) then
   ...
   if (present(ground_rrv)) then
      ...
      write (number,'(f12.3)') ground_rrv * 1000.
      call o_plchhq(xw11,yw2,'ground_rrv = '//trim(adjustl(number)),psiz,0.,0.)
...
if (present(canrrv)) then

   write (number,'(f12.6)') canrrv
   call o_plchhq(.18,yc2 - .082,'canrrv = '//trim(adjustl(number)),psiz,0.,0.)

endif

if (present(canrrv)) then

   write (number,'(f12.6)') canrrv * can_depth * rhos
...
! Print values in vegetation
      ...
      if (present(qshed)) then

         write (number,'(f12.1)') qshed
